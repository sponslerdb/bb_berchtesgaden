...
net <- read_csv("../processed_data/network.csv") %>%
  ...
  group_by(site) %>%
  mutate(period = cut_interval(dayofyear, 4, labels = c("early", "mid1", "mid2", "late"))) # divide each elev.class2 into 4 equal time slices
...
samples <- net %>%
  ...
  group_by(site) %>%
  mutate(period = cut_interval(dayofyear, 4, labels = c("early", "mid1", "mid2", "late"))) %>%
...
# Plot PCoA ordination with arrow for tongue length envfit()
...
niche_analysis <- function(x, y, criterion, ...) { 
  ...
  x %>%
    ...
    mutate(comm1 = map(data, # spread into the kind of array vegan likes
           ...
           envfit.r2 = map(envfit, function(x) x$vectors$r),
           envfit.p = map(envfit, function(x) x$vectors$pvals),
    ...
    unnest(cols = c(ordisurf.dev_exp, ordisurf.p, envfit.r2, envfit.p, adonis.r2, adonis.p, MDS1, MDS2)) %>% # these are lists of length = 1, so unnesting them simply displays the value
    select(..., comm2, traits, pcoa, ordisurf, ordisurf.dev_exp, ordisurf.p, envfit, envfit.fort, pcoa.fort, pcoa.plot, envfit.r2, envfit.p, adonis.r2, adonis.p, MDS1, MDS2)
}

ordi1 <- ordisurf(niche.tax$pcoa[[1]] ~ pbl.w, data = niche.tax$traits[[1]])
...
# Playing with ordisurf as alternative to envfit()
library(ggordiplots)

## Based on https://oliviarata.wordpress.com/2014/07/17/ordinations-in-ggplot2-v2-ordisurf/
# Get ordination object
ordi1 <- niche.tax$ordisurf[[1]] 
ordi1.grid <- ordi1$grid #extracts the ordisurf object
ordi1.mite <- expand.grid(x = ordi1.grid$x, y = ordi1.grid$y) #get x and ys
ordi1.mite$z <- as.vector(ordi1.grid$z) #unravel the matrix for the z scores
ordi1.mite.na <- data.frame(na.omit(ordi1.mite)) %>% #gets rid of the nas
  as_tibble() %>%
  rename(MDS1 = x, MDS2 = y)
  

# Get scores
scores <- fortify(niche.tax$pcoa[[1]], axes = 1:2) %>%
  mutate(Score = case_when(
    Score == "species" ~ "FL.sp",
    Score == "sites" ~ "BB.sp")
    ) %>%
...
# Plot
dat <- filter(scores, (abs(MDS1) > 0.25 | abs(MDS2) > 0.25) | Score == "BB.sp")

ggplot(dat, aes(MDS1, MDS2, pch = Score)) +
  geom_contour(data = ordi1.mite.na, aes(MDS1, MDS2, z = z, color = stat(level)), size = 1.5, alpha = 0.75, inherit.aes = FALSE) +
  geom_point(size = 2) +
...
niche_high_mid1.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "high" & period == "mid1")$pcoa.plot[[1]] 
niche_high_mid2.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "high" & period == "mid2")$pcoa.plot[[1]] 
...
niche_mid_mid1.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "mid" & period == "mid1")$pcoa.plot[[1]] 
niche_mid_mid2.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "mid" & period == "mid2")$pcoa.plot[[1]]
...
niche_low_mid1.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "low" & period == "mid1")$pcoa.plot[[1]]
niche_low_mid2.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "low" & period == "mid2")$pcoa.plot[[1]]
...
niche_high_mid1.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "high" & period == "mid1")$pcoa.plot[[1]] 
niche_high_mid2.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "high" & period == "mid2")$pcoa.plot[[1]] 
...
niche_mid_mid1.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "mid" & period == "mid1")$pcoa.plot[[1]]
niche_mid_mid2.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "mid" & period == "mid2")$pcoa.plot[[1]] 
...
niche_low_mid1.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "low" & period == "mid1")$pcoa.plot[[1]] 
niche_low_mid2.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "low" & period == "mid2")$pcoa.plot[[1]] 
