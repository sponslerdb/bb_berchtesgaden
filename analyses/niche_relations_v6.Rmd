...
pcoa_plot <- function(x, y, min_score = 0.25) {
  ggplot(filter(x, (abs(MDS1) > min_score | abs(MDS2) > min_score) | (Score %in% c("BB.sp", "T.len")))) + # to avoid overplotting, we can filter to include only floral species > loading threshold
...
niche_analysis <- function(x, y, criterion, ...) { 
  ...
  x %>%
    ...
    select(..., comm2, traits, pcoa, ordisurf, ordisurf.dev_exp, ordisurf.p, ordisurf.plot, envfit, envfit.fort, pcoa.fort, pcoa.plot, envfit.r2, envfit.p, adonis.r2, adonis.p, MDS1, MDS2)
}
...
niche.tax.surfplot <- niche.tax$ordisurf.plot
nich.tax.baseplot <- plot(niche.tax$ordisurf[[1]])
niche.tax$ordisurf.plot
plot(niche.tax$ordisurf[[1]])
