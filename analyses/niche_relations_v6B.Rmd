---
title: "Floral resource choice by bumble bee species in the Berchtesgadener Alps"
...
# Do we see the bimodal abundance pattern predicted by the core-satellite hypothesis? Well, sort of.
bb_abund <- net %>%
  group_by(bb.sp) %>%
  summarize(abund = n())

ggplot(bb_abund, aes(abund)) +
  geom_histogram(binwidth = 250)
```

We will parse the samples by elevation class and season, dividing the samples from each site into equal-interval quarters
```{r}
samples <- net %>%
...
surfplot <- function(x, y, min_score = 0.25) {
  ...
  scores <- y %>% 
    #filter((abs(MDS1) > 0.25 | abs(MDS2) > 0.25) | Score == "BB.sp")
    filter((abs(MDS1) > 0.1 | abs(MDS2) > 0.1) | Score == "BB.sp")
  
  ggplot(scores, aes(MDS1, MDS2, pch = Score)) +
    geom_contour(data = contours, aes(MDS1, MDS2, z = z, color = stat(level)), size = 1.5, alpha = 0.75, inherit.aes = FALSE) +
    geom_point(size = 2) +
...
niche_analysis <- function(x, y, criterion, ...) { 
  ...
  x %>%
    filter(caste != "male") %>%
    #filter(!bb.sp %in% c("psit", "gers", "humi")) %>% # omit parasitic species, gers because it's a specialist, humi because it's so rare
    filter(!bb.sp %in% c("gers", "humi")) %>% # gers because it's a specialist, humi because it's so rare
    #filter(bb.sp %in% c("bss", "pasc", "prat", "hort", "soro", "wurf")) %>% # focal species
    group_by(site, date, bb.sp, !!criterion, ...) %>% # pooling all sites and years
    summarize(freq = n()) %>% # get interaction frequency for each BB-FL pair
    group_by(site, date, bb.sp) %>%
    mutate(prop = freq/sum(freq)) %>%
    group_by(site, date, ...) %>%
    complete(bb.sp, !!criterion, fill = list(prop = 0)) %>% # fill in zeros for unvisited plants
    ...
    mutate(comm1 = map(data, # spread into the kind of array vegan likes
           ...
           ordisurf = map2(pcoa, traits, function(x, y) ordisurf(x ~ pbl.w, data = y, k = 8)),
           #ordisurf = map2(pcoa, traits, function(x, y) ordisurf(x ~ pbl.w, data = y, k = 5)),
...
# Calculate site-wise beta diversity from visitation data
...
# Calculate site-wise beta diversity from survey data
...
diet_breit2 <- function(x, y, z, criterion, indices = c("normalised degree")) {
  ...
  filter(caste != "male") %>%out <- left_join(a, b)
}
...
# By color
niche.color <- niche_analysis(net, bb_traits, criterion = color)
...
# Morphological
niche_x_elev.morph <- niche_analysis(net, bb_traits, criterion = k.type.ss, elev.class2)
...
# Color
niche_x_elev.color <- niche_analysis(net, bb_traits, criterion = color, elev.class2)
...
# niche_high_mid1.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "high" & period == "mid1")$ordisurf.plot[[1]] 
# niche_high_mid2.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "high" & period == "mid2")$ordisurf.plot[[1]]
...
# niche_mid_mid1.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "mid" & period == "mid1")$ordisurf.plot[[1]] 
# niche_mid_mid2.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "mid" & period == "mid2")$ordisurf.plot[[1]]
...
# niche_low_mid1.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "low" & period == "mid1")$ordisurf.plot[[1]]
# niche_low_mid2.tax.plot <- filter(niche_x_elevperiod.tax, elev.class2 == "low" & period == "mid2")$ordisurf.plot[[1]]
...
niche_high_mid1.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "high" & period == "mid1")$ordisurf.plot[[1]] 
niche_high_mid2.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "high" & period == "mid2")$ordisurf.plot[[1]] 
...
niche_mid_mid1.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "mid" & period == "mid1")$ordisurf.plot[[1]]
niche_mid_mid2.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "mid" & period == "mid2")$ordisurf.plot[[1]] 
...
niche_low_mid1.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "low" & period == "mid1")$ordisurf.plot[[1]] 
niche_low_mid2.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "low" & period == "mid2")$ordisurf.plot[[1]] 
niche_low_late.morph.plot <- filter(niche_x_elevperiod.morph, elev.class2 == "low" & period == "late")$ordisurf.plot[[1]] 

# Color
niche_x_elevperiod.color <- niche_analysis(net, bb_traits, criterion = k.type.s, elev.class2, period)

## Extract and name plots
niche_high_early.color.plot <- filter(niche_x_elevperiod.color, elev.class2 == "high" & period == "early")$pcoa.plot[[1]]
niche_high_mid1.color.plot <- filter(niche_x_elevperiod.color, elev.class2 == "high" & period == "mid1")$pcoa.plot[[1]] 
niche_high_mid2.color.plot <- filter(niche_x_elevperiod.color, elev.class2 == "high" & period == "mid2")$pcoa.plot[[1]] 
...
niche_mid_mid1.color.plot <- filter(niche_x_elevperiod.color, elev.class2 == "mid" & period == "mid1")$pcoa.plot[[1]]
niche_mid_mid2.color.plot <- filter(niche_x_elevperiod.color, elev.class2 == "mid" & period == "mid2")$pcoa.plot[[1]] 
...
niche_low_mid1.color.plot <- filter(niche_x_elevperiod.color, elev.class2 == "low" & period == "mid1")$pcoa.plot[[1]] 
niche_low_mid2.color.plot <- filter(niche_x_elevperiod.color, elev.class2 == "low" & period == "mid2")$pcoa.plot[[1]] 
...
visitation_abund.sp <- net %>%
  filter(caste != "male") %>%
  filter(!bb.sp %in% c("psit", "gers", "humi")) %>% # only focal species
  #group_by(elev.class2) %>%
  group_by(site) %>%
  mutate(period = cut_interval(dayofyear, 3, labels = c("early",  "mid", "late"))) %>%
  # mutate(k.type.ss = factor(k.type.ss),
  ...
  mutate(k.type.ss = factor(k.type.ss),
         color = factor(color),
         bb.sp = factor(bb.sp)) %>%
  group_by(period, elev.class2, plant.sp.abb, plant.genus, k.type.ss, color, plant.family, bb.sp) %>%
  summarize(abund = n()) %>%
  group_by(elev.class2, period, bb.sp) %>%
  mutate(prop = abund/sum(abund)) %>% 
  left_join(bb_traits) %>%
...
# Mid
tp.mid1 <- ggplot(filter(visitation_abund.sp, period == "mid1"), 
                 aes(area = prop, fill = k.type.ss, label = plant.sp.abb)) +
  geom_treemap(color = "white") +
  geom_treemap_text(fontface = "italic", color = "black", place = "centre", "grow" = TRUE, alpha = 0.5) +
  scale_fill_manual(values = cols) + 
...
tp.mid1.col <- ggplot(filter(visitation_abund.sp, period == "mid1"), 
                 aes(area = prop, fill = color, label = plant.sp.abb)) +
  geom_treemap(color = "white") +
  geom_treemap_text(fontface = "italic", color = "black", place = "centre", "grow" = TRUE, alpha = 0.5) +
  scale_fill_manual(values = cols2) + 
...
tp.mid2 <- ggplot(filter(visitation_abund.sp, period == "mid2"), 
                 aes(area = prop, fill = k.type.ss, label = plant.sp.abb)) +
  geom_treemap(color = "white") +
  geom_treemap_text(fontface = "italic", color = "black", place = "centre", "grow" = TRUE, alpha = 0.5) +
  scale_fill_manual(values = cols) + 
...
tp.mid2.col <- ggplot(filter(visitation_abund.sp, period == "mid2"), 
                 aes(area = prop, fill = color, label = plant.sp.abb)) +
  geom_treemap(color = "white") +
  geom_treemap_text(fontface = "italic", color = "black", place = "centre", "grow" = TRUE, alpha = 0.5) +
  scale_fill_manual(values = cols2) + 
...
```{r}
vis_sum <- net %>%
  group_by(bb.sp, plant.sp, k.type.ss, color) %>%
  summarize()
