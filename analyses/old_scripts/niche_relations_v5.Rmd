...
net <- read_csv("../processed_data/network.csv") %>%
  ...
  group_by(site) %>%
  mutate(period = cut_interval(dayofyear, 4, labels = c("early", "mid1", "mid2", "late"))) # divide each elev.class2 into 3 equal time slices
...
And we will parse the samples by elevation class and season, dividing the samples from each site into equal-interval thirds
```{r}
samples <- net %>%
...
pcoa_plot <- function(x, min_score = 0.25) {
  ...
  cols <- c("flora" = "gray40", "LT spp." = "#F8766D", "ST spp." = "#00BA38", "robber" = "#619CFF")
  
  ggplot(filter(x, (abs(MDS1) > min_score | abs(MDS2) > min_score) | (Score %in% c("LT spp.", "ST spp.", "robber"))), 
...
niche_analysis <- function(x, y, criterion, ...) { 
  ...
  x %>%
    ...
    unnest(cols = c(MDS1, MDS2)) %>% # these are lists of length = 1, so unnesting them simply displays the value
    # select(..., comm2, pcoa, betadis, betadis.permutest, betadis.p, adonis, 
    #        adonis.r2, adonis.p, betadis.boxplot, pcoa.plot, MDS1, MDS2) 
    select(..., comm2, pcoa, pcoa.plot, MDS1, MDS2)
}
