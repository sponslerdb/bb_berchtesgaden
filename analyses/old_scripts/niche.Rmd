...
niche_overlap.sp <- net %>%
  group_by(bb.sp, plant.sp, site, date, year) %>%
  ...
  select(site, date, year, tidy_dist) %>% # drop intermediate list-columns
  unnest(cols = c(tidy_dist)) %>% # unnest back into a single data frame
  left_join(site_data) %>% # add site data
  select(site, date, year, bb.sp1 = item1, bb.sp2 = item2, distance, elev.mean) %>% # select and rename vars
  ...
  select(site, date, year, bb.sp1, bb.sp2, subgenus1, subgenus2, distance, 
...
niche_overlap.sp.mean <- niche_overlap.sp %>%
  group_by(sp.pair, site, year, elev.mean, pbl.diff, subgenus.diff, 
...
niche_overlap.sp.mean2x <- niche_overlap.sp %>%
  group_by(site, year, date, elev.mean) %>% # Average across species pairs within transects
  summarize(mean.niche.overlap = mean(niche.overlap)) %>% 
  group_by(site, year, elev.mean) %>% # Average across transects
  summarize(mean.niche.overlap = mean(mean.niche.overlap)) %>% 
  left_join(BB_covars.mean) %>%
  left_join(FL_covars.mean)

### Genus-level by transect
niche_overlap.gen <- net %>%
  group_by(bb.sp, plant.genus, site, date, year) %>%
  summarize(freq = n()) %>% # get interaction frequency for each BB-FL pair
  group_by(bb.sp, site, date, year) %>%
  mutate(prop = freq/sum(freq)) %>% # get proportional interaction frequency for each BB-FL pair
  select(-freq) %>% # drop freq variable
  pivot_wider(names_from = plant.genus, values_from = prop) %>% # spread into the kind of array vegan likes
  replace(is.na(.), 0) %>% # replace NAs with zeros
  group_by(site, date, year) %>%
  nest() %>% # nest into list-column
  mutate(data2 = map(data, function(x) column_to_rownames(x, var = "bb.sp")), # column to rownames
...
niche_overlap.gen.mean <- niche_overlap.gen %>%
  group_by(sp.pair, site, year, elev.mean, pbl.diff, subgenus.diff, 
           bss, pasc, wurf, prat, soro, hort, jone, psit, mont, hypn, 
           lapi, muci, gers, mend, pyre, humi) %>%
  summarize(mean.niche.overlap = mean(niche.overlap)) %>%
...
niche_overlap.ktype <- net %>%
  left_join(fl_traits) %>%
  group_by(bb.sp, k.type.s, site, date, year) %>%
  summarize(freq = n()) %>% # get interaction frequency for each BB-FL pair
  group_by(bb.sp, site, date, year) %>%
  mutate(prop = freq/sum(freq)) %>% # get proportional interaction frequency for each BB-FL pair
  select(-freq) %>% # drop freq variable
...
```{r}
ggplot(niche_overlap.sp, aes(year, niche.overlap)) +
  geom_boxplot()

ggplot(niche_overlap.gen, aes(year, niche.overlap)) +
  geom_boxplot()
...
ggplot(niche_overlap.sp.mean, aes(year, mean.niche.overlap)) +
  geom_boxplot()

ggplot(niche_overlap.gen.mean, aes(year, mean.niche.overlap)) +
  geom_boxplot()
...
ggplot(niche_overlap.sp.mean2x, aes(year, mean.niche.overlap)) +
  geom_boxplot()

ggplot(niche_overlap.gen.mean2x, aes(year, mean.niche.overlap)) +
  geom_boxplot()
...
```{r}
ggplot(niche_overlap.sp, aes(subgenus.diff, niche.overlap)) +
  geom_boxplot()
...
ggplot(niche_overlap.sp.mean, aes(subgenus.diff, mean.niche.overlap)) +
  geom_boxplot()
...
```{r}
ggplot(niche_overlap.sp, aes(sqrt(pbl.diff), niche.overlap)) +
  geom_point() +
...
ggplot(niche_overlap.sp.mean, aes(sqrt(pbl.diff), mean.niche.overlap)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4)) 
...
```{r}
ggplot(niche_overlap.sp, aes(elev.mean, niche.overlap)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4)) +
  facet_wrap(~sp.pair) +
  #facet_wrap(~year) +
...
ggplot(niche_overlap.gen, aes(elev.mean, niche.overlap)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4)) +
  facet_wrap(~sp.pair) +
  #facet_wrap(~year) +
...
ggplot(niche_overlap.ktype, aes(elev.mean, niche.overlap)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4)) +
  facet_wrap(~sp.pair) +
  #facet_wrap(~year) +
...
ggplot(niche_overlap.sp.mean, aes(elev.mean, mean.niche.overlap)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4)) +
  facet_wrap(~sp.pair) +
  #facet_wrap(~year) +
...
ggplot(niche_overlap.sp.mean2x, aes(elev.mean, mean.niche.overlap)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4)) +
  facet_wrap(~year)
...
```{r}
# Species level
ggplot(niche_overlap.sp, aes(fl.rich, distance)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 4)) +
  #facet_wrap(~sp.pair) +
  #facet_wrap(~year) +
...
```{r}
ggplot(niche_overlap.sp, aes(yday(date), distance)) +
  geom_point() +
  geom_smooth() +
  #facet_wrap(~sp.pair) +
  ylim(c(0,1))
