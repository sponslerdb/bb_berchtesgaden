...
ggsave("./output/bb_range.pdf", height = 6, width = 6)
...
ggplot(bb_range, aes(reorder(bb.sp, elev.med), elev.mean, color = focal)) +
  geom_line(size = 2, alpha = 0.25) +
  geom_point(aes(size = abund), alpha = 0.5) +
  theme_light(12) +
  scale_size_continuous(name = "Abundance") +
...
envfit_fort <- function(x){
  ...
  mutate(Label = case_when(Label == "pbl.cat" ~ "T.class"),
         Score = case_when(Score == "Vector" ~ "T.class"))
}
...
# Plot PCOA; to avoid overplotting, loadings < min_load are not plotted
pcoa_plot <- function(x, min_load = 0.25) {
  
  t.class <- x %>%
    filter(Score == "BB.sp") %>%
    mutate(T.class = factor(if_else(Label %in% c("hort", "pasc", "wurf"), "long", "short")))
  ...
  ggplot(filter(x, (abs(MDS1) > min_load | abs(MDS2) > min_load) | (Score %in% c("BB.sp", "T.len")))) + # to avoid overplotting, we can filter to include only floral species > loading threshold
    geom_polygon(data = t.class, aes(MDS1, MDS2, linetype = T.class), 
                 color = "gray70", fill = NA, size = 1.25, key_glyph = "abline") +
    geom_point(aes(MDS1, MDS2, color = Score), size = 3, alpha = 0.75) +
...
niche_analysis1 <- function(x, y, criterion) {
  ...
  x %>%
    ...
    group_by(pollen) %>%
    nest() %>%
    mutate(comm1 = map(data, function(x) pivot_wider(x, names_from = !!criterion, values_from = prop)), # spread into the kind of array vegan likes
           comm2 = map(comm1, function(x) replace(x, is.na(x), 0)),
           comm3 = map(comm2, function(x) column_to_rownames(x, var = "bb.sp")),
           pcoa = map(comm3,  function(x) capscale(x ~ 1, distance = "horn")),
           traits = map(comm2, function(x) x %>% 
                          left_join(y) %>% 
                          column_to_rownames(var = "bb.sp") %>% 
                          select(pbl.fac)),
           adonis = map2(comm3, traits, function(x, y) adonis(x ~ pbl.fac, data = y, method = "horn")),
           pcoa.fort = map(pcoa, pcoa_fort),
           pcoa.plot = map(pcoa.fort, pcoa_plot),
           tongue.r2 = map(adonis, function(x) x$aov.tab$R2[[1]]),
           tongue.p = map(adonis, function(x) x$aov.tab$'Pr(>F)'[[1]]),
    ...
    select(pollen, comm2, comm3, traits, pcoa, pcoa.fort, adonis,
           pcoa.plot, tongue.r2, tongue.p, MDS1.varexp, MDS2.varexp) 
}

# This version break it down by elev.class
niche_analysis2 <- function(x, y, criterion) {
  ...
  x %>%
    ...
    mutate(comm1 = map(data, function(x) pivot_wider(x, names_from = !!criterion, values_from = prop)), # spread into the kind of array vegan likes
           ...
           MDS2.varexp = map(pcoa, function(x) x$CA$eig[[2]])) %>%
    unnest(cols = c(tongue.r2, tongue.p, MDS1.varexp, MDS2.varexp)) %>%
    select(pollen, elev.class, comm2, comm3, traits, pcoa, pcoa.fort, adonis,
           pcoa.plot, tongue.r2, tongue.p, MDS1.varexp, MDS2.varexp) 
}

# ...and this version breaks it down by elev.class and period
niche_analysis3 <- function(x, y, criterion) {
  ...
  x %>%
    ...
    mutate(comm1 = map(data, function(x) pivot_wider(x, names_from = !!criterion, values_from = prop)), # spread into the kind of array vegan likes
           ...
           MDS2.varexp = map(pcoa, function(x) x$CA$eig[[2]])) %>%
    unnest(cols = c(tongue.r2, tongue.p, MDS1.varexp, MDS2.varexp)) %>%
    select(pollen, elev.class, period, comm2, comm3, traits, pcoa, pcoa.fort, adonis,
           pcoa.plot, tongue.r2, tongue.p, MDS1.varexp, MDS2.varexp) 
}
...
# By plant.sp
niche.tax <- niche_analysis1(net, bb_traits, criterion = plant.sp.abb)
...
# By ktype
niche.morph <- niche_analysis1(net, bb_traits, criterion = k.type.s)
...
```{r message=FALSE, warning=FALSE, echo = FALSE}
# By plant.sp
niche_x_elev.tax <- niche_analysis2(net, bb_traits, plant.sp.abb)
...
# By ktype
niche_x_elev.morph <- niche_analysis2(net, bb_traits, k.type.s)
...
# By ktype
niche_x_elevperiod.morph <- niche_analysis3(net, bb_traits, k.type.s)
...
## Within LT spp
ggplot(filter(niche_overlap.tax, pbl.fac1 == "long" & pbl.fac2 == "long"), 
       aes(elev.mean, niche.overlap, color = pollen)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 8)) 

ggplot(filter(niche_overlap.morph, pbl.fac1 == "long" & pbl.fac2 == "long"), 
       aes(elev.mean, niche.overlap, color = pollen)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 8)) 

## Within ST spp
ggplot(filter(niche_overlap.tax, pbl.fac1 == "short" & pbl.fac2 == "short"), 
       aes(elev.mean, niche.overlap, color = pollen)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 8)) 

ggplot(filter(niche_overlap.morph, pbl.fac1 == "short" & pbl.fac2 == "short"), 
       aes(elev.mean, niche.overlap, color = pollen)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 8))

## Between ST and LT
ggplot(filter(niche_overlap.tax, pbl.fac.diff == "different"), 
       aes(elev.mean, niche.overlap, color = pollen)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 8)) 

ggplot(filter(niche_overlap.morph, pbl.fac.diff == "different"), 
       aes(elev.mean, niche.overlap, color = pollen)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 8)) 
