...
fl_abund_yday <- gamV(log(fl.abund + 0.001) ~ year +
...
fl_abund_gdd <- gamV(log(fl.abund + 0.001) ~ year +
                    te(gdd.cum, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = fl_abund,
                    method = "REML")

check.gamViz(fl_abund_gdd)
summary(fl_abund_gdd)
print(plot(fl_abund_gdd, allTerms = TRUE), pages = 1)
```

##### Floral richness -- since year does not seem to change the overall pattern (though perhaps the magnitude), I could use a simpler model, perhaps with year as a random intercept effect; some explanat
```{r eval=FALSE, include=FALSE}
fl_rich_yday <- gamV(fl.rich ~ year +
                    te(yday, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = fl_rich,
                    method = "REML")

check.gamViz(fl_rich_yday)
summary(fl_rich_yday)
print(plot(fl_rich_yday, allTerms = TRUE), pages = 1)

fl_rich_gdd <- gamV(fl.rich ~ year +
                    te(gdd.cum, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = fl_rich,
                    method = "REML")

check.gamViz(fl_rich_gdd)
summary(fl_rich_gdd)
print(plot(fl_rich_gdd, allTerms = TRUE), pages = 1)

```

##### BB abundance
```{r eval=FALSE, include=FALSE}
bb_abund_yday <- gamV(sqrt(bb.abund) ~ year +
                    te(yday, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = abund_div,
                    method = "REML")

check.gamViz(bb_abund_yday)
summary(bb_abund_yday))
print(plot(bb_abund_yday, allTerms = TRUE), pages = 1)


bb_abund_gdd <- gamV(sqrt(bb.abund) ~ year +
                    te(gdd.cum, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = abund_div,
                    method = "REML")

check.gamViz(bb_abund_gdd)
summary(bb_abund_gdd)
print(plot(bb_abund_gdd, allTerms = TRUE), pages = 1)
```
...
### 2010
bb_abund_sp_yday_2010_nb <- gamV(bb.abund ~ te(yday, elev.mean, 
                                             by= bb.sp, 
                                             bs= c("tp", "tp"), 
                                             k=c(8, 8), m=1),
...
bb_abund_sp_yday_2010 <- gamV(bb.abund ~ te(yday, elev.mean, 
                                             by= bb.sp, 
                                             bs= c("tp", "tp"), 
                                             k=c(8, 8), m=1),
                    data = filter(bb_abund_sp, year == "2010"),
...
print(plot(bb_abund_sp_yday_2010, allTerms = TRUE), pages = 1)

### 2011
bb_abund_sp_yday_2011 <- gamV(bb.abund ~ te(yday, elev.mean, 
                                             by= bb.sp, 
                                             bs= c("tp", "tp"), 
                                             k=c(8, 8), m=1),
...
print(plot(bb_abund_sp_yday_2011, allTerms = TRUE), pages = 1)


### 2012
bb_abund_sp_yday_2012 <- gamV(bb.abund ~ te(yday, elev.mean, 
                                             by= bb.sp, 
                                             bs= c("tp", "tp"), 
                                             k=c(8, 8), m=1),
...
print(plot(bb_abund_sp_yday_2012, allTerms = TRUE), pages = 1)


### This time, we'll try modeling the interaction of year and bb.sp in one model. bam() + discrete=TRUE + fREML might speed things up. I hope.

bb_abund_sp_yday_pool <- bam(bb.abund ~ year + te(yday, elev.mean, 
                                          by= interaction(bb.sp, year), 
                                          bs= c("tp", "tp"), 
                                          k=c(10, 10), m=1),
                    data = bb_abund_sp,
                    discrete = TRUE,
...
bb_abund_sp_yday_pool_vis <- getViz(bb_abund_sp_yday_pool)
check.gamViz(bb_abund_sp_yday_pool_vis)
summary(bb_abund_sp_yday_pool_vis)
print(plot(bb_abund_sp_yday_pool_vis, allTerms = TRUE), pages = 19)
```

##### FL abundance by k.type
##### Need to go back through to simplify, include only ktypes that are used heavily
```{r}
fl_abund_ktype <- survey %>%
  group_by(site, date, dayofyear, gdd.cum, year, elev.class2, 
           elev.mean, break1.gdd, break2.gdd, k.type.ss) %>%
  summarize(fl.abund = sum(flower.cover)) %>%
  rename("yday" = "dayofyear")

fl_abund_ktype_yday_pool <- bam(log(fl.abund + 0.001) ~ year + te(yday, elev.mean, 
                                          by= interaction(k.type.ss, year), 
                                          bs= c("tp", "tp"), 
                                          k=c(10, 10), m=1),
                    data = fl_abund_ktype,
                    discrete = TRUE,
...
fl_abund_ktype_yday_pool_vis <- getViz(fl_abund_ktype_yday_pool)
check.gamViz(fl_abund_ktype_yday_pool_vis)
summary(fl_abund_ktype_yday_pool_vis)
print(plot(fl_abund_ktype_yday_pool_vis, allTerms = TRUE), pages = 10)
```

##### BB richness
```{r eval=FALSE, include=FALSE}
bb_rich_yday <- gamV(bb.rich ~ year +
                    te(yday, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = abund_div,
                    family = "poisson",
                    method = "REML")

check.gamViz(bb_rich_yday)
summary(bb_rich_yday)
print(plot(bb_rich_yday, allTerms = TRUE), pages = 1)
...
ggplot(abund_div, aes(year, bb.rich)) +
  geom_boxplot() +
  facet_wrap(~year)

bb_rich_gdd <- gamV(bb.rich ~ year +
                    te(gdd.cum, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = abund_div,
                    family = "quasipoisson",
                    method = "REML")

check.gamViz(bb_rich_gdd)
summary(bb_rich_gdd)
print(plot(bb_rich_gdd, allTerms = TRUE), pages = 1)
```
...
### Species level
#### BBs
spmet_2012_BB <- rare_net_sp_BB(net_2012, 9, 100) %>%
  mutate(year = factor(rep("2012", n())))
spmet_2011_BB <- rare_net_sp_BB(net_2011, 4, 100) %>%
  mutate(year = factor(rep("2011", n())))  
spmet_2010_BB <- rare_net_sp_BB(net_2010, 3, 100) %>%
...
#### plants
spmet_2012_FL <- rare_net_sp_FL(net_2012, 9, 100) %>%
  mutate(year = factor(rep("2012", n())))
spmet_2011_FL <- rare_net_sp_FL(net_2011, 4, 100) %>%
  mutate(year = factor(rep("2011", n())))  
spmet_2010_FL <- rare_net_sp_FL(net_2010, 3, 100) %>%
  mutate(year = factor(rep("2010", n())))
...
netmet_rare_2012 <- rare_net_nt(net_2012, n = 9, iter = 25) %>%
  mutate(year = factor(rep("2012", n())))
netmet_rare_2011 <- rare_net_nt(net_2011, n = 4, iter = 25)  %>%
  mutate(year = factor(rep("2011", n())))
netmet_rare_2010 <- rare_net_nt(net_2010, n = 3, iter = 25) %>%
  mutate(year = factor(rep("2010", n())))
...
gam_d1 <- gamV(value ~ s(year, bs = "re") + s(elev.mean, k = 5, bs = "tp", by = year, m = 1), 
...
### Species level
spmet_2012_k <- rare_net_sp_k.type.s(net_2012, 9, 20) %>%
  mutate(year = factor(rep("2012", n())))
spmet_2011_k <- rare_net_sp_k.type.s(net_2011, 4, 20) %>%
  mutate(year = factor(rep("2011", n())))  
spmet_2010_k <- rare_net_sp_k.type.s(net_2010, 3, 20) %>%
  mutate(year = factor(rep("2010", n())))
...
### Network- and group-level
netmet_rare_2012_k <- rare_net_nt_k.type.s(net_2012, n = 9, iter = 20) %>%
  mutate(year = factor(rep("2012", n())))
netmet_rare_2011_k <- rare_net_nt_k.type.s(net_2011, n = 4, iter = 20)  %>%
  mutate(year = factor(rep("2011", n())))
netmet_rare_2010_k <- rare_net_nt_k.type.s(net_2010, n = 3, iter = 20) %>%
  mutate(year = factor(rep("2010", n())))
...
##### H2
ggplot(filter(netmet, metric == "H2"), aes(elev.mean, value)) +
  geom_point(aes(color = year)) +
...
kugler_desc <- read_delim("./kugler.tsv", delim = "\t") %>%
  select(1,2)
...
chisq_mod
  ...
  c <- net %>%
  select(k.type.s, pbl.w.class) %>%
  mutate(k.type.s = factor(k.type.s),
         pbl.w.class = factor(pbl.w.class)) %>%
  chisq_test(k.type.s ~ pbl.w.class)
