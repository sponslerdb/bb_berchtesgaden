...
aov.fam <- aov(representation ~ plant.family, data = families)
aov.fam.test <- anova_test(aov.fam)
aov.fam.tuk <- tukey_hsd(aov.fam)

aov.fam2 <- aov(representation.fold ~ plant.family, data = filter(families, !is.infinite(representation.fold)))
aov.fam2.test <- anova_test(aov.fam2)
aov.fam2.tuk <- tukey_hsd(aov.fam2)
...
aov.morph <- aov(representation ~ k.type.ss, data = morphotypes)
aov.morph.test <- anova_test(aov.morph)
aov.morph.tuk <- tukey_hsd(aov.morph)

aov.morph2 <- aov(representation.fold ~ k.type.ss, data = filter(morphotypes, !is.infinite(representation.fold)))
aov.morph2.test <- anova_test(aov.morph2)
aov.morph2.tuk <- tukey_hsd(aov.morph2)
