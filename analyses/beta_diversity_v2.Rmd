...
array_site.date.2010 <- webs2array(net2010) # convert to array of webs
array_site.date.2011 <- webs2array(net2011) # convert to array of webs
array_site.date.2012 <- webs2array(net2012) # convert to array of webs
...
### GDM analysis. I don't think p-values would add anything to the interpretation I get from the plotted uncertainty, so it's okay that I can't get gdm.varImp() to work.
```{r}
### 2010
sitepair_2010.WN <- beta_site.date %>%
  ungroup() %>%
  filter(year1 == year2 & year1 == 2010) %>%
  filter(metric == "WN") %>%
...
### 2011
sitepair_2011.WN <- beta_site.date %>%
  ungroup() %>%
  filter(year1 == year2 & year1 == 2011) %>%
  filter(metric == "WN") %>%
  mutate(weights = rep(1, n())) %>%
  dplyr::select(distance = value, weights,
         s1.xCoord = lon1, s1.yCoord = lat1, 
...
### 2012
sitepair_2012.WN <- beta_site.date %>%
  ungroup() %>%
  filter(year1 == year2 & year1 == 2012) %>%
  filter(metric == "WN") %>%
  mutate(weights = rep(1, n())) %>%
  dplyr::select(distance = value, weights,
         s1.xCoord = lon1, s1.yCoord = lat1, 
