...
no_plant.sp <- niche_overlapper(visit_cover, plant.sp)
...
no_ktype <- niche_overlapper(visit_cover, k.type.ss)
...
no_plant.family <- niche_overlapper(visit_cover, plant.family)
...
ggplot(bias_sitedate, aes(dprime, mean.niche.overlap, color = criterion)) +
...
ggplot(bias_sitedate, aes(dprime, max.niche.overlap, color = criterion)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm") +
  #geom_smooth(method = "gam", formula = y ~ s(x, k = 5), se = FALSE) +
  facet_wrap(~bb.sp) +
...
ggplot(bias_sitedate, aes(log(total.flower.cover), dprime, color = criterion)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm") +
  #geom_smooth(method = "gam", formula = y ~ s(x, k = 5), se = FALSE) +
  facet_wrap(~bb.sp) +
...
ggplot(bias_sitedate, aes(log(total.flower.cover), mean.niche.overlap, color = criterion)) +
  geom_point(alpha = 0.1) +
  geom_smooth(method = "lm") +
  #geom_smooth(method = "gam", formula = y ~ s(x, k = 5), se = FALSE) +
  facet_wrap(~bb.sp) +
...
brm.bias_niche <- brm(mean.niche.overlap ~ 1 + dprime + (1 | bb.sp),
                     cores = 3,
                     data = filter(bias_sitedate, criterion == "ktype"))
...
ggplot(bias_sitedate, aes(yday, dprime, color = criterion)) +
  geom_point(alpha = 0.25) +
  geom_smooth(se = TRUE)
...
ggplot(bias_sitedate, aes(bb.sp.abund, dprime, color = criterion)) +
  #geom_point() +
  geom_smooth() +
  facet_wrap(~bb.sp, )
...
gam.ktype <- bam(dprime ~ s(bb.sp, bs = "re") + 
                 ...
                 method = "fREML",
                 discrete = TRUE,
                 data = filter(bias_sitedate, criterion ==  "ktype")) %>% getViz()
...
inla.pen <- inla(dprime ~ 1 + elev.mean,
  data = filter(bias_sitedate, criterion ==  "ktype"), control.predictor = list(compute = TRUE))
summary(inla.pen)
```
...
## 2.1 Calculate bias per plant speces/ktype/family
...
```{r}
# Function for calculating bias per group (= criterion)
...
ggplot(bias_ktype, aes(criterion, bias)) +
  geom_boxplot()
...
ggplot(bias_ktype, aes(bb.sp, bias)) +
  geom_boxplot() +
  facet_wrap(~criterion)

ggplot(bias_ktype, aes(elev.mean, bias)) +
  #geom_point() +
  geom_smooth(method = gam, formula = y ~ s(x, k = 5)) +
  facet_wrap(~criterion)

# Bias by plant family
ggplot(bias_family, aes(criterion, bias)) +
  geom_boxplot()
...
ggplot(bias_family, aes(elev.mean, bias)) +
  #geom_point() +
  geom_smooth(method = gam, formula = y ~ s(x, k = 5)) +
  facet_wrap(~criterion)
