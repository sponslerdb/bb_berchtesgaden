...
### Would these models be better specified using gamm4? Would that let me handle the crossed and nested random effects better?
...
```{r}
### A gamm4 version
weighted_fl_abundance.gamm4 <- gamm4(log(fl.abund.wt + 0.001) ~
                                       t2(yday, elev.mean, 
                                          by = bb.sp,
                                          #id = 0,
                                          bs= c("gp", "tp"), 
...
weighted_fl_abundance.2010b <- bam(fl.abund.wt + 0.001 ~
                                      s(bb.sp, bs = "re") +
                                      #s(management, bs = "re") +
                                      #s(site, bs = "re") +
                                      te(yday, elev.mean, 
...
check.gamViz(weighted_fl_abundance.2010b)
...
print(plot(weighted_fl_abundance.2010b, allTerms = TRUE), pages = 4)

plot(sm(weighted_fl_abundance.2010, 2))
plot(sm(weighted_fl_abundance.2010b, 2))

weighted_fl_abundance.2010.ck <- check2D(weighted_fl_abundance.2010, 
                                         x1 = "yday", x2 = "elev.mean")
weighted_fl_abundance.2010b.ck <- check2D(weighted_fl_abundance.2010b, 
                                         x1 = "yday", x2 = "elev.mean")
