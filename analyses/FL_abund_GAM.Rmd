...
possible_plants <- bind_rows(dplyr::select(net, c(site, date, plant.sp)),
  ...
  semi_join(survey, by = c("site", "date"))
...
fl_abund_wt <- fl_abund_sp %>%
  ...
  filter(bb.total.abund >= 5) %>% # It's not worth trying to model really rare species
...
```{r}
### Pooled years
weighted_fl_abundance.tw <- bam(fl.abund.wt ~ s(bb.sp, bs = "re") + s(management, bs = "re") + year + 
                                  s(site, bs = "re") +
                                  te(yday, elev.mean, 
                                     by = bb.sp,
                                     bs= c("tp", "tp"), 
...
check.gamViz(weighted_fl_abundance.tw)
weighted_fl_abundance.tw.sum <- summary(weighted_fl_abundance.tw)
print(plot(weighted_fl_abundance.tw, allTerms = TRUE), pages = 1)
saveRDS(weighted_fl_abundance.tw, "../analyses/output/weighted_fl_abundance.rds")

### 2010
weighted_fl_abundance.2010.tw <- bam(fl.abund.wt ~ s(bb.sp, bs = "re") + s(management, bs = "re") + 
                                       s(site, bs = "re") +
                                       te(yday, elev.mean, 
                                          by = bb.sp,
                                          bs= c("tp", "tp"), 
...
check.gamViz(weighted_fl_abundance.2010.tw)
weighted_fl_abundance.2010.tw.sum <- summary(weighted_fl_abundance.2010.tw)
print(plot(weighted_fl_abundance.2010.tw , allTerms = TRUE), pages = 1)

### 2011
weighted_fl_abundance.2011.qp <- bam(fl.abund.wt ~ s(bb.sp, bs = "re") + s(management, bs = "re") + 
                                       s(site, bs = "re") +
                                       te(yday, elev.mean, 
                                          by = bb.sp,
                                          bs= c("tp", "tp"), 
...
weighted_fl_abundance.2011.qp.sum <- summary(weighted_fl_abundance.2011.qp)
print(plot(weighted_fl_abundance.2011.qp , allTerms = TRUE), pages = 1)

### 2012
weighted_fl_abundance.2012.qp <- bam(fl.abund.wt ~ s(bb.sp, bs = "re") + s(management, bs = "re") + 
                                       s(site, bs = "re") +
                                       #slope.est + 
                                       te(yday, elev.mean, 
                                          by = bb.sp,
...
weighted_fl_abundance.2012.qp.sum <- summary(weighted_fl_abundance.2012.qp)
print(plot(weighted_fl_abundance.2012.qp , allTerms = TRUE), pages = 1)
```
...
Does preference relate to BB-FL traits
```{r}
...
ggplot(pref_traits, aes(k.type.s, preference)) +
  geom_boxplot() 

ggplot(filter(pref_traits, preference > 0), aes(k.type.s, log(preference))) +
  geom_boxplot() 

ggplot(pref_traits, aes(tongue.mismatch, preference)) + 
  geom_point() +
  geom_smooth()

ggplot(pref_traits, aes(abs(pbl.w - corolla.depth), preference)) + 
  geom_point() +
  geom_smooth(method = "lm")
...
### Then, to focus only on non-zero preference...
ggplot(filter(pref_traits, preference > 0), aes(tongue.mismatch, log(preference))) + 
  geom_point() +
  geom_smooth(method = "lm")

### Model
library(DHARMa)
lmer_nonzero <- lmer(log(preference) ~ tongue.mismatch + 
                       k.type.s +
                       (1 | plant.sp) +
                       (1 | bb.sp), 
                     data = filter(pref_traits, preference > 0 & !is.na(corolla.depth)))

summary(lmer_nonzero)
plot(lmer_nonzero)
simulationOutput <- simulateResiduals(fittedModel = lmer_nonzero, plot = T)
testResiduals(simulationOutput)
plot(simulationOutput)

lmer_withzeros <- glmer(preference ~ tongue.mismatch + 
                          k.type.s +
                          (1 | plant.sp) +
                          (1 | bb.sp), 
                       family = "poisson",
                       data = filter(pref_traits, !is.na(corolla.depth)))

summary(lmer_withzeros)
plot(lmer_withzeros)
simulationOutput_withzeros <- simulateResiduals(fittedModel = lmer_withzeros, plot = T)
testResiduals(simulationOutput_withzeros)
plot(simulationOutput_withzeros)
