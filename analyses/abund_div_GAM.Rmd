...
```{r}
survey_samples <- survey %>%
  ungroup() %>%
  left_join(site_data) %>%
  mutate(yday = yday(date),
         year = year(date)) %>%
  select(year, yday, elev.mean) %>%
  unique() %>%
  mutate(sample = factor(c(1:length(year)))) %>%
  group_by(year, sample) %>%
  arrange(yday)
%>%
  nest()

dat <- survey_samples$data[[1]]

func <- function(x, days, meters) {
  for(i in 1:dim(x)[1]) {
...
dat <- survey_samples$data[[1]]
yday <- dat$yday # set reference yday
ydays <- seq((yday - 7), (yday + 7), 0.5) # add all days within range
elev <- dat$elev.mean # set reference elev
elevs <- seq((elev - 100), (elev + 100), 1) # add all elevs within range
out.temp <- expand.grid(ydays, elevs) # expand grid to all possible combos
...
### Fill func
#### Creates a set of yday*elevation combinations that can be used to filter surface plots to exclude predictions too far from the data
#### This is very hard-wired right now, and it would break if any of the column names changed.
plotting_boundaries <- function(x,
                                year_sub, # a sloppy way to let me filter by year
                                days, # how many days of outside our samples will we plot?
                                meters) { # how many meters of outside our samples will we plot?
  
  #out <- tibble() # create empty tibble to fill
  out <- c()
  ...
  for(i in 1:dim(data)[1]) { 
    ...
    dat <- x$data[[i]] # this pull out the ith data frame of the data column in the nested data frame
  
    yday <- dat$yday # set reference yday
    # ydays <- seq((yday - days), (yday + days), 0.5) # add all days within range
    # 
    # elev <- dat$elev.mean # set reference elev
    # elevs <- seq((elev - meters), (elev + meters), 1) # add all elevs within range
    # 
    # out.temp <- expand.grid(ydays, elevs) # expand grid to all possible combos
    # out <- bind_rows(out, out.temp)
    out[i] <- yday
  }
  # out <- unique(out) %>% # drop duplicates created by overlapping ranges
  #   select(yday = 1, elev.mean = 2) # name columns
  out
}

FL_plotting_2010 <- plotting_boundaries(survey_samples, 2010, 7, 100)
FL_plotting_2011 <- plotting_boundaries(survey_samples, 2011, 7, 100)
FL_plotting_2012 <- plotting_boundaries(survey_samples, 2012, 7, 100)
View(unique(FL_plotting_2012$yday))

BB_plotting_2010 <- plotting_boundaries(bb_samples, 2010, 7, 100)
BB_plotting_2011 <- plotting_boundaries(bb_samples, 2011, 7, 100)
BB_plotting_2012 <- plotting_boundaries(bb_samples, 2012, 7, 100)
```

### Extract predictions
```{r message=FALSE}
### Species names and such for plotting
species_names <- cbind(c("bss", "pasc", "prat", "soro", "wurf", 
                         "hort", "pyre", "mend", "mont", "gers", 
                         "hypn","jone", "lapi", "muci", "psit", "himi"),
                       c("B. sensu-strictu", "B. pascuorum", "B. pratorum", 
                         ...
                         "B. lapidarius", "B. mucidus", "B. psithyrus", "B. humilis"),
                       c("B. ss.", "B. pasc.", "B. prat.", 
                         "B. soro.", "B. wurf.", "B. hort.",
                         "B. pyre.", "B. mend.", "B. mont.",
                         "B. gers.", "B. hypn.", "B. jone.", 
                         "B. lapi.", "B. muci.", "B. psit.", "B. humi"),
                       c("major", "major", "major", "major", "major",
                         "major", "high elev.", "high elev.", "high elev.",
...
predict2010.FL <- predict_func(weighted_fl_abundance.2010, newdata2010.FL, 2010) %>%
  mutate(yday = ceiling(yday*2)/2) %>%
  semi_join(FL_plotting_2010, by =  c("yday", "elev.mean"))
...
predict2011.FL <- predict_func(weighted_fl_abundance.2011, newdata2011.FL, 2011) %>%
  mutate(yday = ceiling(yday*2)/2) %>%
  semi_join(FL_plotting_2011, by =  c("yday", "elev.mean"))
...
predict2012.FL2 <- predict2012.FL %>%
  mutate(yday = ceiling(yday*2)/2) %>%
  semi_join(FL_plotting_2012, by =  c("yday", "elev.mean"))

ggplot(predict2012.FL2, aes(yday, elev.mean, fill = pred)) +
  geom_raster() +
...
```{r}
#### Major
ggplot(filter(predict_pool, abund.class == "major"), aes(date, elev.mean, fill = pred)) +
  geom_raster() +
  geom_contour(aes(z = pred), color = "black", size = 0.25) +
  scale_fill_viridis_c(name = "log(abund.)") +
...
ggsave("./output/GAM_major.png", height = 3, width = 6.5, units = "in")
