...
gdd_year.gam1 <- gam(gdd.total ~ s(elev.mean) + year,
                    data = site_data) %>% getViz()
summary(gdd_year.gam1)
check.gamViz(gdd_year.gam1)
plot(gdd_year.gam1)

# factor smooth model
gdd_year.gam2 <- gam(gdd.total ~ s(elev.mean, year, bs = "fs", k = 5),
                    data = site_data) %>% getViz()
summary(gdd_year.gam2)
check.gamViz(gdd_year.gam2)
plot(gdd_year.gam2)

# AIC
AIC(gdd_year.gam1, gdd_year.gam2)


site_pairwise_distance_t <- site_data %>%
  ungroup() %>%
...
colnames(site_pairwise_distance_t) <- unique(site_data$site)
rownames(site_pairwise_distance_t) <- unique(site_data$site)

site_pairwise_distance <- site_pairwise_distance_t %>%
  rownames_to_column("site1") %>%
  pivot_longer(names_to = "site2", values_to = "distance", -site1)
```
...
survey_bioData <- survey %>%
  ...
  summarize(cover = mean(flower.cover)) %>%
