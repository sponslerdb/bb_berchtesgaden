...
gdm_WN.2012.imp <- gdm.varImp(sitepair_2012.WN, geo = TRUE, 
                              ...
                              outFile = "./output/winteraction_permGDM_2012")
...
interaction_splines <- bind_rows(interaction_splines2010, 
  ...
  mutate(x.var = factor(x.var, levels = c("Geographic (km)", "Elevation (m)", "Day of Year")),
    subject = rep("BB*FL", dim(.)[1]))
```
...
gdm_BB.tab_join <- bind_rows(gdm_BB.2010.tab_join,
 ...
 # pivot_longer(cols = -c(subject, year), names_to = "var", values_to = "value") %>%
  gt() %>%
  tab_header(title = "GDM Model Output") %>%
  tab_spanner(
    label = "Model",
  ...
  tab_spanner(
    label = "Variable p-value",
    columns = vars(geo.pval, elev.pval, yday.pval)
  ) %>% 
  tab_row_group(
    group = "Bumble bee",
    rows = 1:3
