...
fl_beta <- survey %>%
  ...
  replace(is.na(.), 0) %>%
  column_to_rownames("id") %>%
  vegdist(method = "bray", binary = TRUE, upper = TRUE) %>%
  as.matrix() %>%
  as.data.frame() %>%
  rownames_to_column(var = "id1") %>%
...
species_beta <- full_join(bb_beta, fl_beta) %>%
  ...
  dplyr::select(site1, year1, yday1, site2, year2, yday2, FL, BB,
  ...
  filter(year1 == year2 &
           elev.diff >= 0) # this simply deduplicates the data so that site1 vs site2 and site2 vs. site 1 are not both included; only works because elev.diff is unique for each site pair
```
...
g1 <- ggplot(filter(species_beta, site1 != site2 & yday.diff.abs <= 5), aes(elev.diff, beta)) +
  geom_point(alpha = 0.05, pch = 16) +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 6)) +
  facet_grid(group ~ year1) +
  xlab("Elevation difference (m)") +
  ylab("β-diversity")

# BB + FL time diff
g2 <- ggplot(filter(species_beta, site1 == site2 & yday.diff > 0), aes(yday.diff.abs, beta)) +
  geom_point(alpha = 0.05, pch = 16) +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 6)) +
  facet_grid(group ~ year1) +
...
# BB * FL elevation diff
g3 <- ggplot(filter(beta_mit_data, year1 == year2 &
                      site1 != site2 &
                      metric != "S" & 
...
# BB * FL time diff
g4 <- ggplot(filter(beta_mit_data, year1 == year2 &
                      site1 == site2 &
                      metric != "S" & 
                      !metric.class %in% c("OS.partition", "WN.partition")), 
...
WN.hist <- ggplot(filter(beta_mit_data, year1 == year2 &
                           site1 != site2 &
                           metric == "WN"), 
                aes(value)) +
  geom_histogram() +
...
ggsave("./output/WN_hist.png", width = 6.5, height = 3, units = "in")
```
