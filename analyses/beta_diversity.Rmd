...
p1 <- ggplot(filter(beta_mit_data, year1 == year2 &
                site1 != site2 &
                metric != "S" & 
                !metric.class %in% c("OS.partition", "WN.partition") 
                #& yday.diff <= 7
...
p2 <- ggplot(filter(beta_mit_data, year1 == year2 &
                site1 != site2 &
                metric != "S" & 
                !metric.class %in% c("OS.partition", "WN.partition")
               # & elev.diff <= 250
...
png("./output/beta_stack.png", width = 6.5, height = 6.5, units = "in", res = 300)
p1 / p2 + plot_annotation(tag_levels = c("A", "B", "C")) + plot_layout(guides = "collect")
dev.off()
