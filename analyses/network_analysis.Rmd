...
boot_net_sp <- function(net, n, iter) {
  ...
  for(i in 1:iter) {
    temp <- net %>%
      ...
      map(specieslevel, level = "higher", index = c("d", "resource range")) %>%
...
boot_net <- function(net, n, iter) {
  ...
  for(i in 1:iter) {
    temp <- net %>%
      ...
      data.frame() %>% # convert to data frame
      frame2webs() %>% # convert to to bipartite web
      map(networklevel, level = "higher", weighted = TRUE, index = c("niche overlap")) %>% # We probably care mainly about the BB level of the network
      data.frame() %>%
      t() %>%
      data.frame() %>%
      rownames_to_column() %>%
...
wma1_2010 <- filter(net, site == "wma1", year == "2010")
...
# bb.transects was zeroed out by select = TRUE, so this plot should reflect the model above perfectly
...
check.gamViz(gam_mean_d.1)
concurvity(gam_mean_d.1, full = TRUE)
concurvity(gam_mean_d.1, full = FALSE)
summary(gam_mean_d.1)
print(plot(gam_mean_d.1, allTerms = TRUE), pages = 1)

ggplot(netmet, aes(elev.mean, d)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "tp", k = 10)) +
  facet_wrap(~year)

ggplot(netmet, aes(bb.transects, d)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~year, scales = "free_x")

# I would be reluctant to interpret the year effect biologically; more likely an artifact of sampling intensity
ggplot(netmet, aes(year, d)) +
  geom_boxplot()

# It is interesting that, while elevation explains a lot of niche overlap and virtually none of specialization, niche overlap and specialization are strongly correlated with each other.
ggplot(netmet, aes(niche.overlap.HL, d)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~year)
...
check.gamViz(gam_mean_d.2)
concurvity(gam_mean_d.2, full = TRUE)
concurvity(gam_mean_d.2, full = FALSE)
summary(gam_mean_d.2)
print(plot(gam_mean_d.2, allTerms = TRUE), pages = 1)

ggplot(netmet, aes(bb.rich, d)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~year)

ggplot(netmet, aes(log.bb.abund, d)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~year)

ggplot(netmet, aes(log.fl.abund, d)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~year)

ggplot(netmet, aes(fl.rich, d)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~year)
```
