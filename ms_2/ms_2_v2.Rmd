...
Rather than considering various floral morphological traits independently, we rely on the synthesis of @Kugler1970-gb, who developed a typology consisting of ten primary morphotypes and nested subdivision of each. Kugler’s typology was informed by laboratory studies of bumble bee flower choice [@Kugler1943-uy], and there is a precedent for analyzing bumble bee visitation in terms of Kugler’s morphotypes [@Neumayer1999-il; @Schneller2014-ry]. 
...
While our study spanned three years, we chose not to model year effects. Differences in sampling intensity and duration across years, for the reasons described above, would make comparisons across years difficult to interpret, and we deemed these comparisons not worth the cost of added model complexity. This decision entails the caveats that our analysis cannot detect year-to-year variation in floral biases and niche overlap, and main effects cannot be distinguished from annual anomalies. With only three years of data, though, such annual inferences would be of little value. 
...
### Model 5: Niche overlap by level

### Model 6: Genus-level overlap by species-pair

### Model 7: Community-mean genus-level niche overlap by elevation

We analyze both pairwise niche overlap and community mean niche overlap, since low pairwise niche overlap could conceal a high potential for diffuse competition [@Pianka1974-qk; MacArthur1972-qz]

### Model 8: Pairwise genus-level niche overlap by elevation

## Software

All analyses were conducted in `R` `r citep(citation())`. Bayesian multilevel models were implemented in Stan (Stan Development Team 2021) via the package `brms` `r citep(citation("brms"))`, and visualizations were constructed with the packages `tidybayes` `r citep(citation("tidybayes"))` and `ggdist` `r citep(citation("ggdist"))`. Ordination analysis and permutation testing were performed with the package `vegan` `r citep(citation("vegan"))` and visualized using the packages `ggplot2` `vegan` `r citep(citation("ggplot2"))` and `ggvegan` (Simpson 2019). GAMs were implemented with the package `mgcv` [@Wood_2017] and visualized with the package `mgcViz` [@Fasiolo2018aa]. Data handling and visualization was performed with the `tidyverse` suite of R packages `r citep(citation("tidyverse"))`. Annotated R code is available in the Supplementary Material.
