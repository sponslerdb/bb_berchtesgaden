...
site_data <- read_csv("../data/processed_data/site_data.csv") %>%
  ...
  mutate(elev.class2 = factor(elev.class2, levels = c("low", "mid", "high")),
         site = factor(site),
         tree_line = factor(tree_line)) %>% # turn elav.class into an ordered factor and site into a regular factor
...
# Call GDMs
gdm_survey <- gdm(survey_sitepair, geo = FALSE)
summary(gdm_survey)
...
```{r}
# Function for tabulate gdm.varImp output 
