...
fl_betapart <- survey %>%
  ...
  select(-count) %>%
  pivot_wider(names_from = plant.sp, values_from = present) %>%
  replace(is.na(.), 0) %>%
  column_to_rownames("site") %>%
  beta.pair(index.family = "jaccard") 

fl_betapart.turnover <- fl_betapart$beta.jtu %>%
  as.matrix() %>%
  as.data.frame() %>%
  rownames_to_column(var = "site1") %>%
  pivot_longer(cols = -site1, names_to = "site2", values_to = "tu") %>%
  filter(site1 != site2) %>%
  left_join(site_data, by = c("site1" = "site")) %>%
  dplyr::select(site1, site2, tu, tree_line1 = tree_line) %>%
  left_join(site_data, by = c("site2" = "site")) %>%
  dplyr::select(site1, site2, tu, tree_line1, tree_line2 = tree_line) 

fl_betapart.nestedness <- fl_betapart$beta.jne %>%
  as.matrix() %>%
  as.data.frame() %>%
  rownames_to_column(var = "site1") %>%
  pivot_longer(cols = -site1, names_to = "site2", values_to = "ne") %>%
  filter(site1 != site2) %>%
  left_join(site_data, by = c("site1" = "site")) %>%
  dplyr::select(site1, site2, ne, tree_line1 = tree_line) %>%
  left_join(site_data, by = c("site2" = "site")) %>%
  dplyr::select(site1, site2, ne, tree_line1, tree_line2 = tree_line) 

fl_betapart.collate <- full_join(fl_betapart.turnover, fl_betapart.nestedness) %>%
  mutate(tree_line.comp = case_when(
    tree_line1 == "below" & tree_line2 == "below" ~ "bb",
    tree_line1 == "above" & tree_line2 == "above" ~ "aa",
    tree_line1 != tree_line2 ~ "ab"
  )) %>%
  pivot_longer(cols = c(tu, ne), values_to = "value", names_to = "var")

ggplot(fl_betapart.collate, aes(var, value)) +
  geom_boxplot() +
  facet_wrap(~tree_line.comp)
```
