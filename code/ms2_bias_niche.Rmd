...
visitation <- read_csv("../data/processed_data/network.csv") %>%
  ...
  filter(!bb.sp %in% c("humi", "hypn")) %>% # drop rare species
...
ggplot(brm_pvis_00_plotframe1, aes(condition_mean_trans, tongue, 
...
visitation2 <- read_csv("../data/processed_data/network.csv") %>%
  filter(caste != "male") %>% # we will ignore males 
  filter(!bb.sp %in% c("hypn", "humi")) %>% # hypn and humi are too rare
