...
```{r}
ggplot(null_tab1, aes(ktype, preference)) +
...
ggsave("../ms_2/figures/null_model_ktype_preference.pdf", height = 6, width = 7)
ggsave("../ms_2/figures/null_model_ktype_preference.png", height = 6, width = 7)
```
...
ggplot(gam1_pred, aes(pbl.w, fit)) +
  ...
  #scale_color_manual(values = c("gray40", "#F8766D", "#00B6EB")) +
  facet_wrap(~ktype) +
  labs(x = "Tongue length (mm)", y = "Bias", 
       color = "Species") +
  theme_light()

ggsave("../ms_2/figures/null_model_ktypeXtonguelength_preference.pdf", height = 6, width = 7)
ggsave("../ms_2/figures/null_model_ktypeXtonguelength_preference.png", height = 6, width = 7)
```
...
ggplot(filter(null_tab3_join,
              bb.sp %in% c("hort", "prat", "telu", 
                           "wurf", "soro", "pasc") &
                ktype %in% c("bell", "lip", "flag", "head")), aes(cover, preference)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "lm") +
  facet_grid(bb.sp~ktype, scales = "free") +
  ylim(c(-1, 1))
...
raw_visits_pbl_GAM <- ggplot(gam1b_pred, aes(pbl.w, fit)) +
  ...
  geom_point(data = visitation_prop, # Original data
             aes(pbl.w, mean.prop.visits, color = reorder(bb.sp, pbl.w)), 
             inherit.aes = FALSE, alpha = 0.5) +
  facet_wrap(~ktype) +
...
ggsave("../ms_2/figures/ktypeXtonguelength_raw_visitation.pdf", pbl_GAM_plot, height = 5, width = 8.5)
ggsave("../ms_2/figures/ktypeXtonguelength_raw_visitation.png", pbl_GAM_plot, height = 5, width =8.5)
```
...
# Ordisurf on tongue length
ordisurf_visits <- ordisurf(pca_visits, tongues2$pbl.w) 
