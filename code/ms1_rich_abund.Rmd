---
title: "Appendix S2: Richness and abundance analysis"
output: 
  pdf_document:
    toc: true
    toc_depth: 3
...
site_data <- read_csv("../data/processed_data/site_data.csv") %>%
  dplyr::select(-c(slope.calc, slope.est, elev.min, elev.max, # drop unused columns
...
survey <- read_csv("../data/processed_data/floral_survey.csv") %>%
  # Consider only visited plant species 
...
Bombus  
  - Melissa Broussard
  - http://phylopic.org/image/750eeb4f-7ca5-49ff-a814-27a12d0cb13a/ 
  - https://creativecommons.org/licenses/by/3.0/

Asteraceae
  - http://phylopic.org/image/39335c0c-f879-4df6-90d5-e4f65d90d04e/
...
These are helper functions for tabulating the output of GAMs.
```{r message=FALSE}
gam_tab1 <-function(x1, x2, x3, 
  ...
  tab1.1 <- x1 %>%
    ...
    mutate(class = rep("parametric", n()),
           ...
           lower95 = round(conf.low, 2),
           upper95 = round(conf.high, 2),
           Model = rep(name1, n())
           )
  ...
  out <- bind_rows(tab1.1, tab1.2, tab2.1, tab2.2, tab3.1, tab3.2) %>%
    select(Model, class, term, estimate, lower95, upper95, edf, p.value) %>%
...
fl_abund_sp <- survey %>%
  ...
  summarize(fl.abund = sum(flower.cover)) %>% # get plant.sp*transect total flower cover; 
                                              # this step really shouldn't be required, 
                                              # but in a few cases, there is more than one 
                                              # abundance entry for the same species on a given transect
  group_by(site, date) %>%
...
# Plot year effect
ggplot(gam1.1_year, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic() 

# Plot management effect
ggplot(gam1.1_mgmt, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()
```
...
# Plot year effect
ggplot(gam2.1_year, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()

# Plot management effect
ggplot(gam2.1_mgmt, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()
```

### 3.1.3 Interaction richness
```{r message=FALSE}
# Sanity check visualization
...
# Plot year effect
ggplot(gam3.1_year, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()

# Plot management effect
ggplot(gam3.1_mgmt, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()
```
...
### 3.1.4 Compile plots
```{r message=FALSE}
gam_plot_richness_ <- ( gam1.1_plot1 / gam2.1_plot1 / gam3.1_plot1 ) +
  plot_annotation(tag_levels = 'A') +
  plot_layout(nrow = 3) 

gam_plot_richness <- ggdraw() +
  draw_plot(gam_plot_richness_) +
  draw_image(BB, scale = 0.2, x = 0.07, y = 0.3) +
  draw_image(FL, scale = 0.125, x = 0.07, y = -0.025) +
  draw_image(BBxFL, scale = 0.35, x = 0.07, y = -0.35) 
  
gam_plot_richness_

ggsave("../ms_1/figures/fig2.pdf", gam_plot_richness, width = 4, height = 6.5)
```

## 3.2 Model peak abundance ~ elevation
...
# Plot year effect
ggplot(gam4.1_year, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()

# Plot management effect
ggplot(gam4.1_mgmt, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()
```
...
# Plot year effect
ggplot(gam5.1_year, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()

# Plot management effect
ggplot(gam5.1_mgmt, aes(value, partial)) +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_point() +
  theme_classic()
```
...
### 3.2.3 Compile plots
```{r message=FALSE}
gam_plot_abundance_ <- ( gam4.1_plot1 / gam5.1_plot1 ) +
  plot_annotation(tag_levels = 'A') +
  plot_layout(nrow = 2) 

gam_plot_abundance <- ggdraw() +
  draw_plot(gam_plot_abundance_) +
  draw_image(BB, scale = 0.25, x = 0.07, y = 0.15) +
  draw_image(FL, scale = 0.18, x = 0.07, y = -0.3)
  
gam_plot_abundance_

ggsave("../ms_1/figures/fig3.pdf", gam_plot_abundance, width = 4, height = 6.5)
```

### 3.2.4 BB sp-level abundance ~ elevation
...
gam6.1_elev <- evaluate_smooth(gam6.1, smooth = "s(elev.mean)") %>%
  ...
  # as per reviewer's request for full species names
  mutate(bb.sp.full = case_when(
    bb.sp ==  "pyre" ~ "B. pyrenaeus",
    bb.sp ==  "mend" ~ "B. mendax",
    bb.sp ==  "mont" ~ "B. monticola",
    bb.sp ==  "muci" ~ "B. mucidus",
    bb.sp ==  "soro" ~ "B. soroeensis",
    bb.sp ==  "wurf" ~ "B. wurflenii",
    bb.sp ==  "gers" ~ "B. gerstaeckeri",
    bb.sp ==  "lapi" ~ "B. lapidarius",
    bb.sp ==  "jone" ~ "B. jonellus",
    bb.sp ==  "telu" ~ "B. terrestris/lucorum",
    bb.sp ==  "hort" ~ "B. hortorum",
    bb.sp ==  "hypn" ~ "B. hypnorum",
    bb.sp ==  "prat" ~ "B. pratorum",
    bb.sp ==  "pasc" ~ "B. pascuorum",
    bb.sp ==  "humi" ~ "B. humilis",
    bb.sp ==  "psit" ~ "B. psithyrus"
  ))
...
gam6.1_plot1 <- ggplot(gam6.1_elev, aes(elev.mean, est)) +
  ...
  facet_wrap(~reorder(bb.sp.full, -diff)) +
  theme(strip.text = element_text(face = "italic", size = 8))

ggsave("../ms_1/figures/fig4.pdf", gam6.1_plot1, height = 6.5, width = 6.5)
```

## 3.3 Compile tabulated summaries
...
gam_tab.abund_sp <- bind_rows(gam_tab.abund_sp1, gam_tab.abund_sp2) %>%
    select(class, term, estimate, lower95, upper95, edf, p.value) %>%
    mutate(term = case_when(
      term == "(Intercept)" ~ "Intercept",
      term == "mgmtnone" ~ "mgmt:none",
      term == "mgmtmowing" ~ "mgmt:mowing",
...
## 3.5 Model sample-wise abundance ~ elevation * time  
These models are complex and potentially sensitive to sampling patterns; they should be interpreted cautiously.  
