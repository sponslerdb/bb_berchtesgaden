...
niche_overlapper <- function(x, criterion) {
  ...
  x %>%
    ...
    dplyr::select(site, bb.sp1, bb.sp2, subgenus1, subgenus2, distance, 
       tongue1, tongue2, elev.mean) %>%
    mutate(tongue.diff = as.logical(if_else(tongue1 == tongue2, FALSE, TRUE)),
...
niche_pool <- bind_rows(niche_overlap_gen,
  ...
  mutate(level = factor(level, levels = c("morphotype", "family", "genus")),
         tongue.diff = tongue.diff,
         site = factor(site)) 

write_csv(niche_pool, "../hpc_packets/niche_pool.csv")

niche_pool_sitemean <- niche_pool %>%
  group_by(site, level, elev.mean) %>%
  summarize(mean.niche.overlap = mean(niche.overlap))

# morph_genus_difference <- niche_overlap_gen %>%
#   select(-niche.overlap, -level) %>%
#   full_join(niche_overlap_fam) %>%
#   select(-niche.overlap, -level) %>%
#   full_join(niche_overlap_ktype) %>%
#   select(-niche.overlap, -level) %>%
#   mutate(tongue.diff = tongue.diff,
#          site = factor(site),
#          morph.genus.diff = niche.overlap.ktype - niche.overlap.genus) 
# 
# ggplot(morph_genus_difference, aes(elev.mean, morph.genus.diff)) +
#   geom_point() +
#   geom_smooth() +
#   facet_wrap(~tongue.pair)
# 
# ggplot(morph_genus_difference, aes(tongue.pair, morph.genus.diff)) +
#   geom_boxplot()
```

## 3.5 Niche overlap ~ level
```{r}
...
ggplot(brm_niche00_plotframe1, aes(level, value_trans, 
                                 ymin = .lower_trans, ymax = .upper_trans)) +
  geom_pointinterval() +
  theme_light(16) +
  labs(x = "Level", y= "Niche overlap") +
  guides(fill = FALSE) +
  ylim(c(0, 0.55))
...
gen_gam_niche.I <- bam(niche.overlap ~
                          ...
                          select = TRUE,
                          data = filter(niche_pool, level == "genus" &
                                          bb.sp1 %in% c("pasc", "prat", "soro", "telu", "wurf", # dominant species
                                                        "mont", "muci", "pyre", "mend") & # high elevation specialists
                                          bb.sp2 %in% c("pasc", "prat", "soro", "telu", "wurf",
                                                        "mont", "muci", "pyre", "mend"))) %>% getViz() 

check.gamViz(gen_gam_niche.I)
summary(gen_gam_niche.I)
plot(sm(gen_gam_niche.I, 9))
plot(sm(gen_gam_niche.I, 12))
plot(sm(gen_gam_niche.I, 14))
plot(sm(gen_gam_niche.I, 15))
plot(sm(gen_gam_niche.I, 16))
plot(sm(gen_gam_niche.I, 32))
...
mean_niche_gam.I <- gam(mean.niche.overlap ~
                            level + 
                            s(elev.mean, by = level, k = 8, m = 2),
                          method = "REML",
                          family = betar(eps = 0.001),
                          select = TRUE,
                          data = niche_pool_sitemean) %>% getViz()

check.gamViz(mean_niche_gam.I)
summary(mean_niche_gam.I)
print(plot.gamViz(mean_niche_gam.I, allTerms = TRUE), pages = 1)
```