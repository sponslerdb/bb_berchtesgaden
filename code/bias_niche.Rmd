...
ggsave(file = "../output/brm_pvis00_plot1.png", width = 6, height = 5)
```
...
ggsave(file = "../output/brm_pvis02_plot1.png", width = 8, height = 8)
```
...
ggsave(file = "../output/brm_pvis01_plot1.pdf", width = 6, height = 7)
...
fl_traits2 <- fl_traits %>%
  ...
  mutate(k.type.ss = factor(k.type.ss)) %>%
  distinct() %>%
  na.omit() # THIS IS JUST A PLACEHOLDER UNTIL I FIX THE GENTIANA/ELLA/OPSIS NOMENCALTURE
...
ggplot(pcoa_horn2.fort2, aes(MDS1, MDS2, color = pbl.w.class2)) +
  ...
  geom_text_repel(data = filter(pcoa_horn2.fort1, abs(MDS1) >= 0.22 | abs(MDS2) >= 0.22), 
            aes(MDS1, MDS2, label = plant.genus), inherit.aes = FALSE, color = "gray40") +
  geom_point(data = filter(pcoa_horn2.fort1, abs(MDS1) >= 0.22 | abs(MDS2) >= 0.22),
...
ggsave("../output/bb_range.pdf", width = 6, height = 5)
```

## 3.2 Prep data for calculating niche overlap
```{r}
niche_data <- visitation %>%
  #filter(!bb.sp %in% c("humi", "muci", "gers")) %>%
  left_join(fl_traits) %>%
...
niche_overlap_sppair <-  niche_overlap %>%
  group_by(site, sp.pair, level, elev.mean) %>%
  summarize(mean.niche.overlap = mean(niche.overlap)) %>%
  mutate(level = factor(level, levels = c("family", "morphotype", "genus"))) %>%
  filter(level == "genus") %>%
  separate(sp.pair, c("sp1", "sp2"), sep = ":", remove = FALSE)
...
niche_overlap_tongues <- niche_overlap %>%
  group_by(site, level, tongue.pair, elev.mean) %>%
  summarize(mean.niche.overlap = mean(niche.overlap)) %>%
  filter(level == "genus") 

# Isolate genus-level, then pool within tongue.diff and site
niche_overlap_tonguediff <- niche_overlap %>%
  group_by(site, level, tongue.diff, elev.mean) %>%
  summarize(mean.niche.overlap = mean(niche.overlap)) %>%
  filter(level == "genus")

#write_csv(niche_pool, "../hpc_packets/niche_pool.csv")
...
niche_gam00 <- gam(mean.niche.overlap ~
                     level + 
...
check.gamViz(niche_gam00)
summary(niche_gam00)
print(plot.gamViz(niche_gam00, allTerms = TRUE), pages = 1)
```
...
niche_gam00_pred <- get_gam_predictions(niche_gam00, elev.mean) %>%
  as_tibble() %>%
...
# Plot smooths
ggplot(niche_gam00_pred, aes(elev.mean, mean.niche.overlap, color = level)) +
  geom_point() +
...
ggsave(file = "../output/niche_gam00_plot1.pdf", width = 5, height = 4)
ggsave(file = "../output/niche_gam00_plot1.png", width = 5, height = 4)

# Plot intercept effect of level
plot(pterm(niche_gam00, 1))

plotDiff(sm(niche_gam00, 1), sm(niche_gam00, 3)) +
  l_ciPoly() + 
  l_fitLine() + 
...
### 3.7.1 Model
```{r}
niche_gam20 <- gam(mean.niche.overlap ~
                     tongue.diff +
...
check.gamViz(niche_gam20)
summary(niche_gam20)
print(plot(niche_gam20, allTerms = TRUE), pages = 1)
```
...
```{r}
# Get predictions
niche_gam20_pred <- get_gam_predictions(niche_gam20, elev.mean) %>%
  as_tibble() %>%
  mutate(mean.niche.overlap_trans = inv_logit_scaled(mean.niche.overlap),
         CI_upper_trans = inv_logit_scaled(CI_upper),
         CI_lower_trans = inv_logit_scaled(CI_lower))
...
# Plot smooths
ggplot(niche_gam20_pred, aes(elev.mean, mean.niche.overlap_trans, color = tongue.diff)) +
  geom_ribbon(aes(elev.mean, mean.niche.overlap_trans, 
                  ymax = CI_upper_trans, ymin = CI_lower_trans,
                  fill = tongue.diff),
              alpha = 0.3, inherit.aes = FALSE) +
...
ggsave(file = "../output/niche_gam20_plot1.pdf", width = 5, height = 4)
ggsave(file = "../output/niche_gam20_plot1.png", width = 5, height = 4)

# Plot intercept effect of tongue.diff
plot(pterm(niche_gam20, 1))
```
...
### 3.8.1 Models
```{r}
ggplot(filter(niche_overlap_sppair, !sp1 %in% c("humi") & !sp2 %in% c("humi")), 
