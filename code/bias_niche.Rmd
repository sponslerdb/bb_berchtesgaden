...
conditional_effects(brm_niche10)
...
```{r}
# Beta models
gen_gam_niche.Ib <- bam(niche.overlap ~
                            s(sp.pair, bs = "re") +
                            s(elev.mean, by = sp.pair, k = 6, m = 2),
                          method = "fREML",
...
check.gamViz(gen_gam_niche.Ib)
summary(gen_gam_niche.Ib)
print(plot.gamViz(gen_gam_niche.Ib, allTerms = TRUE), pages = 1)

gen_gam_niche.GI <- bam(niche.overlap ~
                            s(sp.pair, bs = "re") +
                            s(elev.mean, k = 6, m = 2) +
                            s(elev.mean, by = sp.pair, k = 6, m = 1),
                          method = "fREML",
                          family = betar(eps = 0.001),
                          discrete = TRUE,
                          select = TRUE,
...
check.gamViz(gen_gam_niche.GI)
summary(gen_gam_niche.GI)
print(plot.gamViz(gen_gam_niche.GI, allTerms = TRUE), pages = 1)


brm_niche01 <- brm(niche.overlap ~ 1 +
                     s(elev.mean, by = sp.pair, k = 10) +
                     (1 | sp.pair),
                   family = "zero_one_inflated_beta",
                   #file = "../output/brm_niche_01",
                   file = "./brm_niche_01",
                   iter = 2000,
                   chains = 4,
                   data = filter(niche_pool, level == "genus"))
