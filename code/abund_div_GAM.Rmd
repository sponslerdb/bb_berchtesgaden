...
# Just a quick peak at what would happen if I added a by = year smooth 
```{r}
weighted_fl_abund.qp2 <- bam(total.weighted.flower.cover ~ 
                              s(bb.sp, bs = "re") +
                              s(site, bs = "re") +
                              s(year, bs = "re") +
                              te(yday, elev.mean,
...
check.gamViz(weighted_fl_abund.qp2)
sum_fl.qp2 <- summary(weighted_fl_abund.qp2)
print(plot(weighted_fl_abund.qp2, allTerms = TRUE), pages = 4)
check2D(weighted_fl_abund.qp2, x1 = "yday", x2 = "elev.mean")

bb_abund.qp2 <- bam(bb.abund ~ 
                     s(bb.sp.ord, bs = "re") + # center species
                     s(year, bs = "re") + # random intercept for year
                     s(site, bs = "re") + # random intercept to avoid pseudoreplicating elevation
                     te(yday, elev.mean, # tensor product smooth of elevation and time
...
check.gamViz(bb_abund.qp2)
sum.qp2 <- summary(bb_abund.qp2)
print(plot(bb_abund.qp2, allTerms = TRUE), pages = 4)
check2D(bb_abund.qp2, x1 = "yday", x2 = "elev.mean")
```