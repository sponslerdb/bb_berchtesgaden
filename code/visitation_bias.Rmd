...
```{r}
representation <- biaseval_group(visit_cover, plant.sp) %>%
  rename(plant.sp = criterion) %>%
  left_join(site_data) %>%
  left_join(fl_traits) %>%
  mutate(k.type.ss = factor(k.type.ss)) %>%
  select(site, date, elev.mean, yday, bb.sp, plant.sp, k.type.ss, 
         prop.visits, prop.flower.cover, bias, visits, 
         flower.cover, plant.genus, plant.family) %>%
  left_join(bb_traits)
...
```{r}
# The tons of zeros are due to the lots of plant species that were present at very low abundance and received few or zero visits. 
...
ggplot(representation, aes(bias)) +
  geom_histogram() +
  facet_wrap(~k.type.ss)

ggplot(filter(representation, abs(bias) > 0.1), aes(bias)) +
  geom_histogram() +
  facet_wrap(~k.type.ss)
...
ggplot(representation, aes(prop.flower.cover, prop.visits)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~k.type.ss)

brm_representation0 <- brm(bias ~ 1 + (1 | k.type.ss) + (1 | site),
                 control = list(max_treedepth = 10),
                 cores = 4,
                 chains = 4,
                 iter = 2000,
                 file = "../output/brm_representation0",
                 data = representation)

summary(brm_representation0)
plot(brm_representation0)
parnames(brm_representation0)
mcmc_plot(brm_representation0, pars = c("^r_"))
#mcmc_plot(representation.ktype0, pars = c("elev"))
mcmc_plot(brm_representation0, pars = c("^b_"))
mcmc_plot(brm_representation0)
#conditional_effects(representation.ktype0)
pp_check(brm_representation0)
bayes_R2(brm_representation0)


brm_representation1 <- brm(bias ~ pbl.w + (1 + pbl.w | k.type.ss) + (1 | bb.sp),
                 control = list(max_treedepth = 10),
                 cores = 4,
                 chains = 4,
                 iter = 2000,
                 file = "../output/brm_representation1",
                 data = representation)

summary(brm_representation1)
plot(brm_representation1)
parnames(brm_representation1)
mcmc_plot(brm_representation1, pars = c("^r_"))
#mcmc_plot(representation.ktype1, pars = c("elev"))
mcmc_plot(brm_representation1, pars = c("^b_"))
mcmc_plot(brm_representation1)
#conditional_effects(representation.ktype1)
pp_check(brm_representation1)
bayes_R2(brm_representation1)

hypothesis(representation.ktype1, "r_group[5,Intercept] = r_group[6,Intercept]", class = "")
hypothesis(representation.ktype1, "r_group[1,Intercept] = r_group[0,Intercept]", class = "")


# One interesting way to think about bias is to focus on the slope of prop.visits ~ prop.flower.cover. 
# We could analyze this with a varying slope model
...
ggplot(representation, aes(sqrt(prop.flower.cover), sqrt(prop.visits))) +
  geom_point() +
  #geom_smooth(method = "lm") +
...
brm_representation3 <- brm(prop.visits ~ prop.flower.cover + (1 + prop.flower.cover | k.type.ss),
                 control = list(max_treedepth = 10),
                 #cores = 4,
                 cores = 2,
                 chains = 2,
                 #iter = 2000,
                 iter = 1000,
                 file = "./output/brm_representation3",
                 data = representation)

lmer_representation3 <- lmer(prop.visits ~ log(prop.flower.cover) + (1 + log(prop.flower.cover) | k.type.ss),
                             data = representation)

summary(lmer_representation3)
plot(lmer_representation3)

# Can we explain anything with tongue length

ggplot(filter(representation, bb.sp != "gers" & abs(bias) >= 0.1), aes(pbl.w, bias)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~k.type.ss)

lm_pbl <- lmer(bias ~ pbl.w + (1 + pbl.w | k.type.ss) + (1 | bb.sp),
               data = representation)
