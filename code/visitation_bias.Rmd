...
# 1. How does overall bias (Blüthgen's d') vary across species and through elev*time? For now, we will focus on bias at the level of plant species. We'll start considering traits later on when our goal is to explain patterns of bias.
...
# This model includes a global interaction term for yday and elevation, with a varying intercept for bb.sp and a varying slow for elev | bb.sp. I think this should give me all the estimation I'm interested in.
...
# This, I think, would also be a legitimate way to specify the model, but it would not tell us anything about elevation or time. 
...
ggplot(representation_ktype, aes(group, bias)) +
  geom_boxplot()
...
ggplot(filter(representation, visits > 0), aes(bias)) +
  geom_histogram() +
  facet_wrap(~k.type.ss)
...
summary(lm(representation ~ k.type.ss, data = bias))

brm_representation0 <- brm(bias ~ 1 + (1 | group),
                 control = list(max_treedepth = 10),
                 cores = 4,
                 iter = 2000,
                 file = "./output/brm_representation1",
                 data = representation_ktype)

brm_representation1 <- brm(bias ~ 1 + (1 | k.type.ss),
                 control = list(max_treedepth = 10),
                 #cores = 4,
                 cores = 4,
                 chains = 4,
...
# 3. How do species vary in their biases?
```{r}
rep_matrix.plantsp <- representation %>%
  group_by(bb.sp, group) %>%
  summarize(bias = mean(bias)) %>%
  pivot_wider(values_from = bias, names_from = group, values_fill = 0) %>%
  column_to_rownames("bb.sp")
...
envfit(pca.plantsp ~ pbl.w.class, data = traits)
summary(ordisurf(pca.plantsp ~ pbl.w.class, data = traits))


pca.ktype <- rda(bias_matrix.ktype)
plot(pca.ktype, type = "t")
envfit(pca.ktype ~ pbl.w.class, data = traits)
summary(ordisurf(pca.ktype ~ pbl.w.class, data = traits))
pca.ktype_pca1_traits <- pca.ktype$CA$u[,1] %>%
  as.data.frame() %>%
  rownames_to_column(var = "bb.sp") %>%
  rename(PC1 = 2) %>%
  left_join(bb_traits)

ggplot(pca.ktype_pca1_traits, aes(pbl.w, PC1)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2))

ggplot(pca.ktype_pca1_traits, aes(pbl.w.class, PC1)) +
  geom_point() +
  geom_smooth(method = "lm")
```
