...
Conversation with Ingolf
- Add opening paragraph setting context of clinate change and community ecology
...
- Ingolf had a REALLY nice title -- ask him to send it to me.
...
- Switch Fabrice and Kalli's order
- Cite Ingolf's recent papers showing the effect of pulses of mass-flowering crop resource on bumble bee health
- He liked my colony phase figures
...
breakpoints <- function(x, year) {
  ...
  break1.low <- getX(glm1.low, 0.5) %>%
    data.frame() %>%
    select(gdd = 1) %>%
    mutate(elev.class2 = "low",
           breakID = "break1")  

  break1.mid <- getX(glm1.mid, 0.5) %>%
    data.frame() %>%
    select(gdd = 1) %>%
    mutate(elev.class2 = "mid",
           breakID = "break1")
  
  break1.high <- getX(glm1.high, 0.5) %>%
    data.frame() %>%
    select(gdd = 1) %>%
    mutate(elev.class2 = "high",
           breakID = "break1")
  
  break2.low <- getX(glm2.low, 0.2) %>%
    data.frame() %>%
    select(gdd = 1) %>%
    mutate(elev.class2 = "low",
           breakID = "break2")  
  
  break2.mid <- getX(glm2.mid, 0.2) %>%
    data.frame() %>%
    select(gdd = 1) %>%
    mutate(elev.class2 = "mid",
           breakID = "break2")
  
  break2.high <- getX(glm2.high, 0.2) %>%
    data.frame() %>%
    select(gdd = 1) %>%
    mutate(elev.class2 = "high",
           breakID = "break2")
  ...
  out <- data %>%
    ...
    gather(caste, count, -c(site, date, year, q.prop, r.prop,
                          gdd.cum, elev.class, elev.class2, 
...
breaks_2012 <- breakpoints(castes, 2012)
breaks_2011 <- breakpoints(castes, 2011)
breaks_2010 <- breakpoints(castes, 2010)
...
ggplot(breaks_2012, aes(gdd.cum, prop, color = caste)) +
  geom_point() +
...
ggsave("./output/phases2012.pdf")

ggplot(breaks_2011, aes(gdd.cum, prop, color = caste)) +
  geom_point() +
  binomial_smooth(formula = y ~ splines::ns(x, 2)) +
  geom_vline(aes(xintercept = break1), linetype = "dashed") +
  geom_vline(aes(xintercept = break2), linetype = "dashed") +
...
ggsave("./output/phases2011.pdf")

ggplot(breaks_2010, aes(gdd.cum, prop, color = caste)) +
  geom_point() +
  binomial_smooth(formula = y ~ splines::ns(x, 2)) +
  geom_vline(aes(xintercept = break1), linetype = "dashed") +
  geom_vline(aes(xintercept = break2), linetype = "dashed") +
