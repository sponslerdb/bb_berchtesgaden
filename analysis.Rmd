...
fl_abund_yday <- gamV(fl.abund ~ year +
                    te(yday, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = abund_div,
                    family = "tw",
                    method = "REML")

fl_abund_yday2 <- gamV(log(fl.abund + 0.001) ~ year +
                    te(yday, elev.mean, by=year, bs= c("cr", "cr"), k=c(10, 10), m=1),
                    data = abund_div,
                    #family = "scat",
                    method = "REML")

fl_abund_yday3 <- gamV(log(fl.abund + 0.001) ~ year +
                    te(yday, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = abund_div,
                    #family = "scat",
                    method = "REML")

AIC(fl_abund_yday, fl_abund_yday2, fl_abund_yday3)
...
check.gamViz(fl_abund_yday2)
summary(fl_abund_yday2)

print(plot(fl_abund_yday2, allTerms = TRUE), pages = 1)


check.gamViz(fl_abund_yday3)
summary(fl_abund_yday3)

print(plot(fl_abund_yday3, allTerms = TRUE), pages = 1)
...
plot(fl_abund_gdd.vis, allTerms = TRUE) +
...
```{r eval=FALSE, include=FALSE}
### By year
fl_rich_yday <- gam(fl.rich ~ year +
                    te(yday, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = filter(abund_div),
                    method = "REML")

fl_rich_yday.vis <- getViz(fl_rich_yday)
check.gamViz(fl_rich_yday.vis)
summary(fl_rich_yday.vis)

plot(fl_rich_yday.vis, allTerms = TRUE) +
  ggtitle("FL richness (gdd)") +
  xlab("Day of year") +
  ylab("Elevation")

### With year as parametric term
fl_rich_yday2 <- gam(fl.rich ~ year +
                    te(yday, elev.mean, bs= "tp", k= 10, m=1),
                    data = filter(abund_div),
                    method = "REML")

fl_rich_yday2.vis <- getViz(fl_rich_yday2)
check.gamViz(fl_rich_yday2.vis)
summary(fl_rich_yday2.vis)

plot(sm(fl_rich_yday2.vis, 1)) + 
  l_fitRaster() + 
  l_fitContour() + 
  l_points(size = 1) +
  ggtitle("FL richness (yday)") +
  xlab("Day of year") +
  ylab("Elevation")

plot(pterm(fl_rich_yday2.vis, 1))


### By year
fl_rich_gdd <- gam(fl.rich ~ year +
                    te(gdd.cum, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = filter(abund_div),
                    method = "REML")

fl_rich_gdd.vis <- getViz(fl_rich_gdd)
check.gamViz(fl_rich_gdd.vis)
summary(fl_rich_gdd.vis)

plot(fl_rich_gdd.vis, allTerms = TRUE) +
  ggtitle("FL richness (gdd)") +
  xlab("GDD") +
  ylab("Elevation")

### With year as parametric term
fl_rich_gdd2 <- gam(fl.rich ~ year +
                    te(gdd.cum, elev.mean, bs= "tp", k= 10, m=1),
                    data = filter(abund_div),
                    method = "REML")

fl_rich_gdd2.vis <- getViz(fl_rich_gdd2)
check.gamViz(fl_rich_gdd2.vis)
summary(fl_rich_gdd2.vis)

plot(sm(fl_rich_gdd2.vis, 1)) + 
  l_fitRaster() + 
  l_fitContour() + 
  l_points(size = 1) +
  ggtitle("FL richness (gdd)") +
...
plot(bb_abund_yday.vis, allTerms = TRUE) +
...
plot(bb_abund_gdd.vis, allTerms = TRUE) +
...
summary(bb_rich_yday.vis)
plot(sm(bb_rich_yday.vis, 1)) +
  l_fitRaster() +
  l_fitContour() +
  l_points() +
  ggtitle("bb richness 2010 (yday)") +
  xlab("Day of year") +
  ylab("Elevation")
plot(sm(bb_rich_yday.vis, 2)) +
  l_fitRaster() +
  l_fitContour() +
  l_points() +
  ggtitle("bb richness 2011 (yday)") +
  xlab("Day of year") +
  ylab("Elevation")
plot(sm(bb_rich_yday.vis, 3)) +
  l_fitRaster() +
  l_fitContour() +
  l_points() +
  ggtitle("bb richness 2012 (yday)") +
  xlab("Day of year") +
  ylab("Elevation")


bb_rich_gdd <- gam(bb.rich ~ year +
                    te(gdd.cum, elev.mean, by=year, bs= c("tp", "tp"), k=c(10, 10), m=1),
                    data = filter(abund_div),
                    method = "REML")
...
summary(bb_rich_gdd.vis)

plot(bb_rich_gdd.vis, allTerms = TRUE) +
...
gam_d <- gamV(value ~ year + s(elev.mean, k = 15, bs = "tp", by = factor(year)), 
             data = filter(spmet_BB, metric == "d"),
             method = "REML")

check.gamViz(gam_d)
summary(gam_d)

plot(gam_d, allTerms = TRUE) 
...
gam_niche.overlap <- gamV(value ~ year + s(elev.mean, k = 10, bs = "tp", by = factor(year)), 
...
ggplot(filter(netmet, metric == "niche.overlap.LL"), aes(elev.mean, value)) +
  geom_point(aes(color = year)) +
...
gam_cscore <- gamV(value ~ year + s(elev.mean, bs = "tp", k = 10, by = factor(year)), 
...
ggplot(filter(netmet, metric == "C.score.LL"), aes(elev.mean, value)) +
  geom_point(aes(color = year)) +
  #geom_smooth(method = "lm", formula = y ~ poly(x, 3)) +
  geom_smooth(method = "gam", formula = y ~ s(x), color = "black", linetype = "dashed", size = 0.75) +
  #geom_smooth() +
