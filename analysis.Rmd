...
fl_traits <- read.csv("./processed_data/floral_k_type.csv") %>%
...
survey_trait_elev <- survey %>%
  group_by(site, year, elev.mean,  k.type) %>%
  summarize(abund = sum(flower.cover)) %>%
  group_by(site, elev.mean, year) %>%
  mutate(div = length(unique(k.type)))
...
ggplot(survey_trait_elev, aes(elev.mean, log(abund))) +
  geom_point() +
  stat_smooth() +
  facet_wrap(~k.type, scales = "free_y")

ggplot(net_trait_elev, aes(elev.mean, log(abund))) +
  geom_point() +
  stat_smooth() +
  facet_wrap(~k.type, scales = "free_y")

ggplot(survey_trait_elev, aes(elev.mean, div)) +
  geom_point() +
  stat_smooth()

ggplot(net_trait_elev, aes(elev.mean, div)) +
  geom_point() +
  stat_smooth()
...
##### Floral richness -- since year does not seem to change the overall pattern (though perhaps the magnitude), I could use a simpler model, perhaps with year as a random intercept effect
```{r eval=FALSE, include=FALSE}
