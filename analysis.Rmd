...
## group level
rare_net_gp <- function(net, n, iter) {
  ...
  for(i in 1:iter) {
    temp <- net %>%
      ...
      unique() %>%
      group_by(site) %>%
      sample_n(n) %>% # drawn n sites at random
      left_join(net) %>%
      group_by(site, bb.sp, plant.sp) %>% # group by site, bee species, plant species
...
rare_net_nt <- function(net, n, iter) {
  ...
  for(i in 1:iter) {
    temp <- net %>%
      ...
      data.frame() %>% # convert to data frame
      frame2webs() %>% # convert to to bipartite web
      map(networklevel, weighted = TRUE) %>% # We probably care mainly about the BB level of the network
      data.frame() %>%
      t() %>%
      data.frame() %>%
      rownames_to_column() %>%
...
ggplot(dens_div, aes(elev.mean, bb.per.fl, color = year)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
...
groupmet_rare_2012 <- rare_net_gp(net_2012, n = 9, iter = 20)
groupmet_rare_2011 <- rare_net_gp(net_2011, n = 4, iter = 20)
groupmet_rare_2010 <- rare_net_gp(net_2010, n = 3, iter = 20)

### Plot
ggplot(filter(groupmet_rare_2012, metric == "niche.overlap.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2011, metric == "niche.overlap.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2010, metric == "niche.overlap.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()


ggplot(filter(groupmet_rare_2012, metric == "C.score.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2011, metric == "C.score.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2010, metric == "C.score.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()


ggplot(filter(groupmet_rare_2012, metric == "V.ratio.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2011, metric == "V.ratio.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2010, metric == "V.ratio.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()


ggplot(filter(groupmet_rare_2012, metric == "discrepancy.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2011, metric == "discrepancy.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2010, metric == "discrepancy.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()


ggplot(filter(groupmet_rare_2012, metric == "generality.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2011, metric == "generality.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(groupmet_rare_2010, metric == "generality.HL"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()
```
...
spmet_2012_mean <- spmet_2012 %>%
  group_by(site, elev.mean, metric) %>%
  summarize(mean.value = mean(value))

spmet_2011 <- rare_net_sp(net_2011, 4, 20) %>%
  group_by(site, elev.mean, iteration) %>%
  summarize(mean.d = mean(d))
spmet_2010 <- rare_net_sp(net_2010, 3, 20) %>%
  group_by(site, elev.mean, iteration) %>%
  summarize(mean.d = mean(d))

ggplot(filter(spmet_2012_mean, metric == "d"), aes(elev.mean, mean.value)) +
  geom_point() +
  geom_smooth() 

ggplot(spmet_2011, aes(elev.mean, mean.d)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)

ggplot(spmet_2010, aes(elev.mean, mean.d)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)
...
### Plot
ggplot(filter(netmet_rare_2012, metric == "H2"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(netmet_rare_2011, metric == "H2"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()

ggplot(filter(netmet_rare_2010, metric == "H2"), aes(elev.mean, value)) +
  geom_point() +
  geom_smooth()


ggplot(netmet_rare_2012, aes(elev.mean, number.of.compartments)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)

ggplot(netmet_rare_2011, aes(elev.mean, number.of.compartments)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)

ggplot(netmet_rare_2010, aes(elev.mean, number.of.compartments)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)

ggplot(netmet_rare_2012, aes(elev.mean, web.asymmetry)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)

ggplot(netmet_rare_2011, aes(elev.mean, web.asymmetry)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)

ggplot(netmet_rare_2010, aes(elev.mean, web.asymmetry)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~iteration)
```
