...
plot(fl_abund_yday.vis, allTerms = TRUE) +
...
fl_rich_yday2 <- gam(fl.rich ~ year +
                    te(yday, elev.mean, bs= "cc", k= 10, m=1),
                    data = filter(abund_div),
                    method = "REML")
...
fl_rich_gdd2 <- gam(fl.rich ~ year +
                    te(gdd.cum, elev.mean, bs= "cc", k= 10, m=1),
                    data = filter(abund_div),
                    method = "REML")
...
#### Community mean d
ggplot(filter(spmet_mean, metric == "d"), aes(elev.mean, value)) +
...
##### Connectance
ggplot(filter(netmet, metric == "weighted.connectance"), aes(elev.mean, value)) +
  geom_point(aes(color = year)) +
  #geom_smooth(method = "lm", formula = y ~ poly(x, 3)) +
  geom_smooth(method = "gam", formula = y ~ s(x), color = "black", linetype = "dashed", size = 0.75) +
  #geom_smooth() +
  xlab("Elevation") +
  ylab("Connectance (weighted)") +
  theme_light(16)

gam_connectance <- gam(value ~ year + s(elev.mean), 
                  data = filter(netmet, metric == "weighted.connectance"),
                  method = "REML")

gam_connectance.vis <- getViz(gam_connectance)
check.gamViz(gam_connectance.vis)
summary(gam_connectance.vis)

plot(gam_connectance.vis, allTerms = TRUE) +
  xlab("Elevation")

##### NODF
ggplot(filter(netmet, metric == "weighted.NODF"), aes(elev.mean, value)) +
  geom_point(aes(color = year)) +
  #geom_smooth(method = "lm", formula = y ~ poly(x, 3)) +
  geom_smooth(method = "gam", formula = y ~ s(x), color = "black", linetype = "dashed", size = 0.75) +
  #geom_smooth() +
...
gam_NODF <- gam(value ~ year + s(elev.mean), 
                data = filter(netmet, metric == "weighted.NODF"),
                method = "REML")

gam_NODF.vis <- getViz(gam_NODF)
check.gamViz(gam_NODF.vis)
summary(gam_NODF.vis)

plot(gam_NODF.vis, allTerms = TRUE) +
  xlab("Elevation")

##### Web asymmetry
ggplot(filter(netmet, metric == "web.asymmetry"), aes(elev.mean, value)) +
  geom_point(aes(color = year)) +
  #geom_smooth(method = "lm", formula = y ~ poly(x, 3)) +
  geom_smooth(method = "gam", formula = y ~ s(x), color = "black", linetype = "dashed", size = 0.75) +
  #geom_smooth() +
...
gam_webass <- gam(value ~ year + s(elev.mean), 
                data = filter(netmet, metric == "web.asymmetry"),
                method = "REML")

gam_webass.vis <- getViz(gam_webass)
check.gamViz(gam_webass.vis)
summary(gam_webass.vis)

plot(gam_webass.vis, allTerms = TRUE) +
  xlab("Elevation")
```
...
### Species level
spmet_2012 <- rare_net_sp_k.type(net_2012, 9, 20) %>%
  mutate(year = factor(rep("2012", n())))
spmet_2011 <- rare_net_sp(net_2011, 4, 20) %>%
  mutate(year = factor(rep("2011", n())))  
spmet_2010 <- rare_net_sp(net_2010, 3, 20) %>%
...
### Network- and group-level
netmet_rare_2012 <- rare_net_nt_k.type(net_2012, n = 9, iter = 20) %>%
  mutate(year = factor(rep("2012", n())))
netmet_rare_2011 <- rare_net_nt_k.type(net_2011, n = 4, iter = 20)  %>%
  mutate(year = factor(rep("2011", n())))
netmet_rare_2010 <- rare_net_nt_k.type(net_2010, n = 3, iter = 20) %>%
  mutate(year = factor(rep("2010", n())))
