...
The study was conducted 2010-2012 in Berchtesgaden National Park, located in the Northern Limestone Alps of southeast Germany (47.55°N, 12.92°E). The landscape is composed of mountain pastures mainly surrounded by coniferous forests. We selected 25 study sites (60 x 60 m) on mountain pastures at elevations ranging from 641-2032 meters above sea level (m.s.l.) **(Figure 1)**. Fourteen of these pastures are extensively grazed by cattle or sheep, 3 are mowed for hay production, and 8 have been abandoned throughout the last century and are no longer subject to any human management. Study sites were classified as falling either above or below the tree line based on an examination of Google Earth imagery and corroborated by the authors with field experience at our study sites (K. Kallnik, A. F. Maihoff, A. Claßen). Among our sites, the tree line generally fell at an elevation of around 1500 m.s.l., consistent with the previous descriptions of the tree line in the Berchtesgander Alps [@Kostler1970-ya; @Mayer1970-zn] **(Figure 2)**. Eight sites fell above the tree line and 17 fell below it **(Figures 1, 2)**.  
...
Bumble bee abundance was quantified as the total number of recorded floral visits per bumble bee species per site-date. To represent floral abundance from the perspective of each bumble bee species in our network, we first scored each plant genus in our data set as either visited or not visited by each bumble bee species. We then calculated the total floral resource availability for each bumble bee species for each site-date by summing the observed flower cover of visited genera. We opted to work at the genus level based on the reasoning that if a bumble bee species visits one member of a given genus, other members of the same genus should also be considered potential floral hosts. This approach was intended to dampen the effects of false non-detection on the estimation of floral resource abundance, particularly for rare bumble bee species whose diet breadth would tend to be underestimated simply due to sparsity of observations [@Williams2005-yp]. We also opted to treat the cryptic *B. terrestris/lucorum* species pair as a single morphospecies, since these two species cannot be visually distinguished reliably. We also pooled representatives of the parasitic *Psithyrus* subgenus --- which included *B. barbutellus*, *B. bohemicus*, *B. campestris*, *B. flavidus*, *B. quadricolor*, and *B. sylvestris* --- into a single species group *B. psithryus*.   


### Elevational distribution of bumble bees and floral resources

We analyzed the abundance of bumble bees and floral resources using separate hierarchical generalized additive models (HGAM) [@Wood2011-pg; @Pedersen2019-kf]. We opted in both models to the collapse temporal variation of abundance within sites and focus on elevational patterns. For bumble bees, we modeled *peak* abundance at each site as a smooth response to elevation, with bumble bee species as a "by" smoothing factor and a random intercept effect. We also included species:year as a random intercept effect to capture annual diffences in species abundance and site as a random intercept effect to avoid overfitting due to pseudoreplication of elevation arising from the measurement of multiple bumble bee species at each site. This construction is a modification of the "type I" model form described in @Pedersen2019-kf. For floral resources, we modeled *mean* flower cover, weighted per bumble be species, as a smooth response to elevation, with bumble bee species and year as "factor-smooths" and site as a random intercept effect (again to avoid overfitting due to pseudoreplication). This construction is a modification of the "type S" model form described in @Pedersen2019-kf.
...
```{r fig1, echo = FALSE,  fig.cap = paste("Proportional abundances (cumulative within site) of bumble bee species in our study system, ordered by mean proportional abundance.")}
...
Our model explained 71.5% of total deviance (adjusted R^2^ = 0.511).
...
```{r fig3, echo = FALSE,  fig.cap = paste("Peak bumble bee abundance by elevation and year. Curves shows the conditional GAM predictions excluding the site random effect, and points show original data. Dotted vertical lines indicate the approximate location of the tree line at our study sites. Note different scales of y-axes.")}
knitr::include_graphics("../output/gam_max_bb_abund2.png")
```

```{r fig4, echo = FALSE,  fig.cap = paste("Mean weighted floral abundance (area of flower cover) by elevation and year. Lines shows the conditional GAM predictions excluding the random effects of site and species, and points show original data. The right panel displays the full range of the data while the left panel displays a subset of the range, omitting extreme values of flower cover to highlight the prediction curves.")}
knitr::include_graphics("../output/gam_mean_fl_abund_zoom.png")
```
